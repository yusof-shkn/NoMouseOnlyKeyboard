...
uv run main.py template health

# Full idempotent seed/repair
uv run main.py template seed

# Only seed sections that are incomplete
uv run main.py template seed --missing
```
...
uv run main.py seed --company-id 5 --all

# Individual steps (can be combined)
uv run main.py seed --company-id 5 --company          # Company record + settings
uv run main.py seed --company-id 5 --copy-template    # Roles, COA, categories from template
uv run main.py seed --company-id 5 --stores           # 3 areas × 2 stores
uv run main.py seed --company-id 5 --suppliers        # 4 suppliers
uv run main.py seed --company-id 5 --customers        # 6 customers
uv run main.py seed --company-id 5 --products         # Load NDA products (read-only)
uv run main.py seed --company-id 5 --inventory        # Batches per store
uv run main.py seed --company-id 5 --purchases        # 3 POs per store
uv run main.py seed --company-id 5 --sales            # ~43 sales per store
uv run main.py seed --company-id 5 --accounting       # Expenses

# ⚠️  Destructive – clears all data for the company
uv run main.py seed --company-id 5 --clear
```
