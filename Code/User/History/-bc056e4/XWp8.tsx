...
// Separate component so each row has its own menu open state
function ActionCell({
  row,
  rowIndex,
  actions,
  ...
  return (
    ...
    >
      <Menu
        ...
        onChange={setOpened}
      >
        {/* Target is just an invisible anchor point — the Td itself is the trigger */}
        <Menu.Target>
          <Box
            style={{
              display: 'flex',
              ...
              width: '100%',
              minHeight: '36px',
              // We stop click here too so the Td onClick doesn't double-fire
              pointerEvents: 'none',
            }}
          >
            <IconDots
...
function TableContent({
  ...
  const getSelectionColor = (isHover: boolean = false) => {
    ...
    if (isHover) {
      return alpha(primaryColor, 0.15) // more intense hover
    }
    return alpha(primaryColor, 0.1) // more intense selected
  }

  const getHoverColor = () => {
    // More intense row hover
    return colorScheme === 'dark' ? theme.colors.dark[4] : theme.colors.gray[2]
  }
