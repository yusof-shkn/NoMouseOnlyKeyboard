...
// ============================================================
// NEW: Invoice metadata (shared with purchaseOrderHistory.Queries)
// ============================================================
 ...
 * Now accepts optional invoiceMeta to persist invoice_number / invoice_date / delivery_notes.
...
export async function receiveGoodsFromPO(
  ...
  // Update status using RPC function
...
/**
 * ✅ UPDATED: createPurchaseOrderWithBatches now accepts invoiceMeta
 * and persists invoice_number / invoice_date when receiveImmediately = true.
 */
export const createPurchaseOrderWithBatches = async (request: {
  ...
  try {
    ...
    // Build insert object — include invoice fields if provided and receiving immediately
    ...
    // (non-fatal: if columns don't exist the insert will still succeed via
    //  the two-step pattern used in the update path)
    ...
    if (poError) {
      // If the error is the missing column, retry without invoice fields
      if (
        ...
        // We'll fall through with retryPO — reassign for the rest of the function
        Object.assign(purchaseOrder ?? {}, retryPO)

        // Attempt the invoice update separately (also non-fatal)
    ...
    // Create PO items with correct quantities
