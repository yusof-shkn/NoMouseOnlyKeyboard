...
// Cap longest edge at 2400 px — all prescription text stays readable.
// Quality 0.88 keeps JPEG sharp while cutting file size ~60–70%.
...
// ── Open camera via a temporary input — most reliable across mobile browsers ──
// Some Android/iOS browsers ignore `capture` on a hidden React-managed input;
// creating a fresh, non-hidden input and clicking it synchronously works reliably.
function openCamera(onFile: (f: File) => void): void {
  const tmp = document.createElement('input')
  tmp.type = 'file'
  tmp.accept = 'image/*'
  tmp.capture = 'environment' // rear camera
  ...
  // Must call click() synchronously inside the same user-gesture tick
  tmp.click()
}
...
// ── Contact option ─────────────────────────────────────────────────────────────
// 'profile'  = the profile's saved phone (selectedAddr-style card)
// 'custom'   = user types their own number
...
export default function UploadPrescription(): JSX.Element {
  ...
  // ── Derived from selected profile ─────────────────────────────────────────
  const activeProfile = profiles.find((p) => p.id === profileId)
  const profilePhone = activeProfile?.phone ?? null
  const defaultAddress = profileAddrs.find((a) => a.is_default) ?? null

  // ── Auto-fill phone when profile changes ──────────────────────────────────
  useEffect(() => {
    if (profilePhone) {
  ...
  }, [profileId, profilePhone, setValue])
  ...
  const handleFile = async (f: File) => {
    ...
    if (!ACCEPTED.some((t) => f.type === t || f.type.startsWith('image/'))) {
      notifications.show({
            ...
            <Stack gap="lg">
              ...
              <Box>
                ...
                {!preview ? (
                  <Stack gap={10}>
                    ...
                    {/* Camera — opens rear camera directly via a fresh DOM input */}
                    <Box
                      onClick={() => openCamera(handleFile)}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: 14,
                    ...
                    >
                      ...
                      <Box style={{ flex: 1 }}>
                        ...
                        <Text
                          ...
                          c="dimmed"
                        >
                          Open rear camera to photograph your prescription
                        </Text>
                      </Box>
                    </Box>
                  </Stack>
                ...
                {/* Gallery input only — no capture attribute */}
                <input
                  ref={fileInputRef}
                  type="file"
              ...
              <Box>
                ...
                {loadingAddrs ? (
                  ...
                  <Stack gap="xs">
                    {/* ── Profile default address — pinned at top ── */}
                    {defaultAddress && (
                      <>
                        <Text
                          size="xs"
                        ...
                        {renderAddrCard({
                          ...
                          instructions: defaultAddress.instructions ?? null,
                          latitude: defaultAddress.latitude ?? null,
                          longitude: defaultAddress.longitude ?? null,
                          is_default: true,
                        })}
                        {(profileAddrs.filter((a) => !a.is_default).length >
                          0 ||
                          historyAddrs.length > 0) && <Divider my="xs" />}
                      </>
                    )}

                    {/* Other saved profile addresses (non-default) */}
                    {profileAddrs.filter((a) => !a.is_default).length > 0 && (
                      <>
                        <Text
                          size="xs"
                          fw={600}
              ...
              <Box>
                ...
                <Stack gap="xs">
                  {/* ── Profile phone — pinned at top when it exists ── */}
                  {profilePhone && (
                    <>
                      <Text
                        size="xs"
                      ...
                      <Box
                        ...
                        style={{
                          ...
                          borderRadius: 10,
                          cursor: 'pointer',
                          border: `2px solid ${contactOption === 'profile' ? SavedColors.Primaryblue : SavedColors.DemWhite}`,
                          background:
                            contactOption === 'profile' ? '#EBF7FD' : '#fff',
                          transition: 'all 0.15s',
                        }}
                      >
                        <Box
                  ...
                  <Box
                    style={{
                      ...
                      background:
                        contactOption === 'custom' ? '#EBF7FD' : '#fff',
                      transition: 'all 0.15s',
                    }}
                  >
                    ...
                    >
                      ...
                      <Box style={{ flex: 1 }}>
                        ...
                        <Text
                          ...
                          c="dimmed"
                        >
                          Enter any number for this order
                        </Text>
                      </Box>
                      <Radio
                        checked={contactOption === 'custom'}
                        onChange={() => setContactOption('custom')}
                        color="lotusCyan"
                        style={{ pointerEvents: 'none' }}
                      />
                    </Box>

                    {/* Input — only visible when this option is selected */}
                    {contactOption === 'custom' && (
                      <Box
                        style={{
                ...
                {errors.contactPhone && contactOption !== 'profile' && (
                  <Text
                    size="xs"
                    c="red"
                    mt={4}
