...
export default function UploadPrescription(): JSX.Element {
  ...
  const onSubmit = async (values: FormValues) => {
    if (!user) return
    // For cash orders, image is optional IF medicine names are provided
    const hasMedicineText = (values.medicineNames ?? '').trim().length > 0
    if (!files.length && !cashOnly) {
      notifications.show({
    ...
    if (!files.length && cashOnly && !hasMedicineText) {
      notifications.show({
        title: 'Order details required',
        message:
          'Please upload a prescription image or type the medicine name(s) you need.',
        color: 'orange',
      })
      return
    }
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          >
            ...
            <Box>
              ...
              >
                {cashOnly
                  ? 'Type the medicine names or upload a prescription image — either works.'
                  : 'Upload your prescription to use insurance. Image is required.'}
              </Text>
              ...
              {/* Medicine name text input — shown when cash selected */}
              {cashOnly && (
                <Box mt={10}>
                  <Text
                    size="xs"
                  ...
                  >
                    {files.length > 0
                      ? 'Or type medicine names (optional)'
                      : 'Type medicine name(s)'}
                    {files.length === 0 && (
                      <Text
                        component="span"
                        c="red"
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Textarea
                        {...field}
                        placeholder={
                          'e.g. Paracetamol 500mg × 2, Amoxicillin 250mg × 1 One medicine per line or comma-separated'
                        }
                        rows={3}
                        radius="md"
                        style={{ fontSize: 13 }}
                  ...
                  <Text
                    ...
                    mt={4}
                  >
                    Include dosage and quantity if you know them.
                  </Text>
                </Box>
              )}
            </Box>
