...
const StockList = () => {
  ...
  const isFetchingRef = useRef(false) // ref mirror — safe to read inside callbacks without deps
  ...
  const fetchStockListOnly = useCallback(async () => {
    if (!companyId || isFetchingRef.current) {
    ...
    try {
      ...
      const {
      ...
      } = await fetchStockListData(
        {
          ...
          companyId,
          isUnlocked,
          storeId: storeIds?.length === 1 ? storeIds[0] : selectedStore?.id,
          storeIds: storeIds,
        },
        companySettings,
      )
  ...
  const rowActions = getStockListRowActions(
    {
      onView: (row) => handleView(row, dispatch),
    ...
    <PageWrapper>
      ...
      >
        ...
        <ContentTable
          ...
          selectable={true}
          onRowClick={(row) => handleView(row, dispatch)}
        />
      </Container>
    </PageWrapper>
  )
