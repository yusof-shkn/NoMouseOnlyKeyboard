  ...
  try {
    ...
    data?.forEach((transaction) => {
      ...
      if (transaction.activity_type === 'operating') {
        stats.operatingCashFlow += transaction.transaction_type === 'inflow' ? amount : -amount
      } else if (transaction.activity_type === 'investing') {
        stats.investingCashFlow += transaction.transaction_type === 'inflow' ? amount : -amount
      } else if (transaction.activity_type === 'financing') {
        stats.financingCashFlow += transaction.transaction_type === 'inflow' ? amount : -amount
      }
    })
...
export const getCashFlowTransactionById = async (
  id: number
): Promise<{ data: CashFlowTransaction | null; error: PostgrestError | null }> => {
  return await supabase
    .from('cash_flow_transactions')
    .select('*')
    .eq('id', id)
