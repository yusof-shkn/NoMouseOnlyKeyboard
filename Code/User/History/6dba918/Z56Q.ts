...
/**
 * Get cash flow statistics — queries cash_flow_transactions directly.
 * Supports optional store and date range filters.
 *
 * Actual table columns used: transaction_type, activity_type, amount,
 * company_id, store_id, transaction_date, deleted_at
 */
 ...
 * Queries cash_flow_transactions directly (NOT the view).
 *
 * Confirmed table columns: id, company_id, store_id, transaction_date,
 * transaction_type, activity_type, description, amount, reference_type,
 * reference_id, reference_number, journal_entry_id, category,
 * payment_method, notes, created_by, created_at, updated_at, deleted_at
 *
 * NOTE: account_id, receipt_number, account_number, bank_name do NOT
 * exist — search uses only description + reference_number.
 */
export const getCashFlowTransactions = async ({
  ...
  storeId = null,
  companyId = null,
  accountId = null, // no account_id on this table — kept for API compat, ignored
  ...
  try {
    ...
    // Search — only columns confirmed to exist on the table
