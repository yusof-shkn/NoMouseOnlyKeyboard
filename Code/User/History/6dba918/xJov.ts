...
export const getCashFlowTransactions = async ({
  ...
  try {
    ...
    let query = supabase
      .from('v_cash_flow') // ✅ Updated view name (was: vw_cash_flow_statement)
      .select('*', { count: 'exact' })
    ...
    // Transaction type filter — v_cash_flow has no transaction_type column.
    // Inflows have cash_in > 0; outflows have cash_out > 0.
 ...
 * NOTE: No updated v_ version exists for this summary view — using vw_ is correct.
 *
...
export const getCashFlowStats = async (
  ...
  try {
    // vw_cash_flow_summary doesn't support date filtering (it's a summary view).
    // For date-filtered stats we would need to query v_cash_flow directly.
 ...
 * Used for delete and detail-view operations where the view is insufficient.
 */
