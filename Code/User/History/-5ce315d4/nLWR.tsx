...
export const ReceiveGoodsForm = ({
  ...
  // Fetch units for this company AND master catalog (company 1) — products may reference
  // company-1 unit IDs, so we need both to resolve names and pre-select defaults.
  useEffect(() => {
    if (!companyId) return
    supabase
      .from('units')
