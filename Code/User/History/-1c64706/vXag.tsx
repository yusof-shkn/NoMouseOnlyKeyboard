...
const CategoryForm: React.FC<CategoryFormProps> = ({
  ...
  // ── Theme tokens (matches PrintBarcode.tsx pattern exactly) ──────────────
  const primaryColor = getPrimaryColor(theme)
  const borderColor = getBorderColor(theme, 'default')
  const surfaceColor = getSurfaceColor(
    theme,
    isDark ? 'elevated' : 'background',
  )
  const selectionBg = getSelectionColor(theme, 'background')

  // Shared input styles reused across every field
  const inputStyles = {
    input: {
      backgroundColor: isDark ? theme.colors.dark[6] : theme.white,
      borderColor,
      color: isDark ? theme.colors.gray[0] : theme.black,
    },
  }
    ...
    return (
      ...
      >
        <Alert
          ...
          styles={{
            root: {
              backgroundColor: isDark ? theme.colors.dark[6] : selectionBg,
            },
          }}
        >
