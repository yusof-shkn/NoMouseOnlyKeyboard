...
const Suppliers = () => {
  ...
  const handleBalanceFilterChange = useCallback((value: string) => {
    setBalanceFilter(value as 'all' | 'with_balance' | 'no_balance')
    setCurrentPage(1)
  }, [])

  const columns = useMemo(() => getSupplierColumns(currency), [currency])

  const filters = useMemo(
    () => getSupplierFilters(handleFilterChange, handleBalanceFilterChange),
    [handleFilterChange, handleBalanceFilterChange],
  )

  const statisticsData = useMemo(
    () => getStatisticsCardsData(stats, currency),
    [stats, currency],
  )
