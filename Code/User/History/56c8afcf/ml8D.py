...
def _seed_parser(sub):
    ...
    p.add_argument("--copy-template", action="store_true", help="Copy template master data")
    ...
    p.add_argument("--products",      action="store_true", help="Load NDA products (no insert)")
    ...
    p.add_argument("--clear",         action="store_true", help="⚠️  Clear all data (destructive)")
    return p
...
def main():
    ...
    tmpl_seed_p.add_argument("--missing", action="store_true",
    ...
    if args.command == "scrape":
        ...
        active_flags = {k: v for k, v in vars(args).items()
                        if k not in ("command", "company_id")}
        if not any(active_flags.values()):
            seed_p.print_help()
            sys.exit(0)
        from src.seeder import run_seed
        run_seed(args)
        ...
        if args.template_cmd == "health":
            ...
            from src.template import run_health_check, SEED_PIPELINE
            from src.utils import log_section, log_step, log_ok
            ...
            if args.missing:
                ...
                pipeline = [(name, fn) for name, fn in SEED_PIPELINE if name in missing_names]
