...
/**
 * Derive transaction_type from v_cash_flow columns (cash_in / cash_out).
 * v_cash_flow does not have a transaction_type column — this helper is used
 * wherever the old code assumed that field existed.
 */
const getTransactionType = (row: CashFlowTransaction): 'inflow' | 'outflow' =>
  row.transaction_type ?? ((row.cash_in ?? 0) > 0 ? 'inflow' : 'outflow')

/**
 * Derive a single amount value from v_cash_flow columns.
 * Prefer the enriched `amount` field when already set; otherwise derive.
 */
const getAmount = (row: CashFlowTransaction): number =>
  row.amount ??
  ((row.cash_in ?? 0) > 0 ? (row.cash_in ?? 0) : (row.cash_out ?? 0))
...
export const handleDelete = async (
  ...
  // receipt_number is on the underlying table; row may not have it when sourced
  // from the view — fall back gracefully.
  const receiptLabel = row.receipt_number
    ? `Receipt #${row.receipt_number}`
    : `Entry: ${row.description || 'N/A'}`
  const confirmDelete = window.confirm(
    `Are you sure you want to delete this transaction? ${receiptLabel}`,
  )
...
export const handleRowClick = (
  ...
  // Derive amount and transaction_type from view columns when not already set
  ...
  if (selectedTransaction && selectedTransaction.id === row.id) {
    ...
    notifications.show({
      ...
      message: `${txType === 'inflow' ? 'Cash In' : 'Cash Out'} — ${formattedAmount}`,
      color: 'blue',
      autoClose: 2000,
    })
  }
...
export const handleViewDetails = (
  ...
  notifications.show({
    ...
    message: `${row.description} — ${row.account_name}`,
    color: 'blue',
  })
}
