...
export const handleDelete = async (
  ...
  // Use reference_number for the confirm dialog label — works for all sources
  const refLabel = row.reference_number
  ...
  const confirmDelete = window.confirm(
    `Are you sure you want to delete ${refLabel}?`,
  )
  if (!confirmDelete) return

  try {
    // deleteCashFlowTransaction only soft-deletes cash_flow_transactions rows.
    // Rows sourced from sales/expenses/purchase_orders cannot be deleted here —
    // they must be managed from their own pages.
...
export const handleRowClick = (
  ...
  // amount is a real column in v_cash_flow — no derivation needed
...
export const handleViewDetails = (
  ...
  // category tells us the source (sale, purchase_order, expense, income, etc.)
  const source = row.category ? ` (${row.category.replace(/_/g, ' ')})` : ''

  notifications.show({
    title: `Transaction Details${source}`,
    message: `${row.description}${row.reference_number ? ` — Ref: ${row.reference_number}` : ''}`,
    color: 'blue',
  })
}
