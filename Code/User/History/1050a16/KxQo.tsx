...
const Sidebar = styled.nav<{ $open: boolean }>`
  width: 240px;
  min-height: 100vh;
  background: ${SavedColors.FooterBgColor};
  position: fixed;
  top: 0;
  left: 0;
  z-index: 200;
...
export default function UserManagement() {
  ...
  const fetchUsers = async () => {
    ...
    const { data, error } = await supabase
      ...
      .not(
        'user_id',
        'in',
        `(${staffIdList.length ? staffIdList.join(',') : 'null'})`,
      )
  ...
  const filterProfiles = (list: ProfileWithInsurance[]) =>
    list.filter(
      (p) =>
        ...
        (p.insurance as any)?.provider
          ?.toLowerCase()
          .includes(search.toLowerCase()) ||
        (p.insurance as any)?.policy_number
          ?.toLowerCase()
          .includes(search.toLowerCase()),
    )
  ...
  const ins = Array.isArray(selected?.insurance)
    ? (selected?.insurance as any[])[0]
    : (selected?.insurance as DbInsurance | undefined)
    ...
    <Shell>
      ...
      <Sidebar $open={sidebarOpen}>
        ...
        >
          ...
          <NavBtn
            style={{
              background: 'rgba(21,179,224,0.15)',
              color: SavedColors.Primaryblue,
          ...
          >
            {[
              ...
              <Box
                ...
                style={{
                  background: gradient
                    ? 'linear-gradient(135deg,#15B3E0,#012970)'
                    : highlight
                      ? '#fffbeb'
              ...
              >
                <Text
                  size="xs"
                  c={gradient ? 'rgba(255,255,255,0.75)' : 'dimmed'}
                >
                  {label}
                </Text>
                <Text
          ...
          {pending.length > 0 && (
            ...
            >
              <Text
                ...
                fw={600}
              >
                {pending.length} insurance record{pending.length > 1 ? 's' : ''}{' '}
                pending review — customers are waiting for approval.
              </Text>
            </Alert>
          )}
              ...
              >
                ...
                <Box>
                  ...
                  <Text
                    ...
                    c="dimmed"
                  >
                    {selected.relationship} · DOB: {selected.date_of_birth}
                  </Text>
                  ...
                  {[
                    ...
                    >
                      ...
                      <Text
                        size="sm"
                        fw={600}
                        c={value === '—' ? 'dimmed' : SavedColors.TextColor}
                      >
                        {value}
                      </Text>
                    </Box>
                    ...
                    {ins.card_front_url && (
                      <Box style={{ flex: 1 }}>
                        ...
                        <Image
                          ...
                          onClick={() =>
                            window.open(ins.card_front_url, '_blank')
                          }
                        />
                      </Box>
                    )}
                    {ins.card_back_url && (
                      <Box style={{ flex: 1 }}>
                        ...
                        <Image
                          ...
                          onClick={() =>
                            window.open(ins.card_back_url, '_blank')
                          }
                        />
                      </Box>
                    )}
              ...
              {ins.status === 'verified' ? (
                ...
                >
                  <Text
                    ...
                    fw={600}
                  >
                    This insurance is already verified. Customer has been
                    notified.
                  </Text>
                </Alert>
              ) : (
    ...
    <Box style={{ overflowX: 'auto' }}>
      ...
      >
        ...
        <Table.Tbody>
          {items.map((p) => {
            const ins = (
              Array.isArray(p.insurance) ? p.insurance[0] : p.insurance
            ) as DbInsurance
            return (
              <Table.Tr key={p.id}>
                <Table.Td>
                ...
                <Table.Td>
                  <Text size="sm">{ins?.provider || '—'}</Text>
                </Table.Td>
                <Table.Td>
                  <Text
                    size="sm"
                ...
                <Table.Td>
                  <Text
                    ...
                    c={
                      ins?.expiry_date &&
                      ins.expiry_date < new Date().toISOString().slice(0, 10)
                        ? 'red'
                        : 'dimmed'
                    }
                  >
