...
const FormPanel = styled.div`
  ...
  @media(max-width:800px){min-height:100vh;padding:0;display:flex;flex-direction:column;justify-content:flex-end;
...
const MobileHero = styled.div<{$lifted?:boolean}>`
  display:none;
  @media(max-width:800px){display:flex;flex-direction:column;align-items:center;position:relative;text-align:center;flex-shrink:0;
    padding:1rem 1.5rem 1.25rem;opacity:${p=>p.$lifted?0:1};pointer-events:${p=>p.$lifted?'none':'auto'};
    transition:opacity 0.2s ease;will-change:opacity;}
`
...
const Card = styled.div<{$lifted?:boolean}>`
  ...
  @media(max-width:800px){max-width:100%;border:none;padding:1.75rem 1.5rem 1.75rem ;
    box-shadow:0 -4px 40px rgba(0,0,0,0.18);border-radius:28px 28px 0 0;
