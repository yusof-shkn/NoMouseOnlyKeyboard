...
const Card = styled.div<{ $lifted?: boolean }>`
  ...
  @media (max-width: 800px) {
    ...
    ${(p) =>
      p.$lifted
        ? `position:fixed;top:0;left:0;right:0;bottom:0;border-radius:0;overflow-y:auto;padding:max(.5rem,calc(env(safe-area-inset-top,0px) + .25rem)) 1.5rem max(5rem,calc(env(safe-area-inset-bottom,0px) + 3rem));z-index:500;box-shadow:none;-webkit-overflow-scrolling:touch;background:#fff;`
        : ''}
  }
`
