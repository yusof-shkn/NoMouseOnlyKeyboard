...
const BrandOrb = styled.div<{
  ...
  background: radial-gradient(
    circle,
    rgba(21, 179, 224, 0.18) 0%,
    rgba(21, 179, 224, 0) 70%
  );
  top: ${(p) => p.$top};
  left: ${(p) => p.$left};
...
`
const FormPanel = styled.div<{ $lifted?: boolean }>`
  background: #f0f4fa;
  display: flex;
  align-items: center;
  justify-content: center;
  ...
  @media (max-width: 800px) {
    ...
    background: ${(p) =>
      p.$lifted
        ? '#fff'
        : 'linear-gradient(165deg,#011a4a 0%,#012970 35%,#0d5fa4 60%,#15b3e0 100%)'};
    ${(p) =>
      p.$lifted
        ? 'position:fixed;top:0;left:0;right:0;bottom:0;z-index:498;'
        : ''}
    &::before {
      display: none;
    }
...
`
const MobileHeroOrb = styled.div<{
  $size: number
  $top: string
  $left: string
}>`
...
const Card = styled.div<{ $lifted?: boolean }>`
  ...
  @media (max-width: 800px) {
    max-width: 100%;
    border: none;
    padding: 1.75rem 1.5rem 10.75rem 1.5rem;
    box-shadow: 0 -4px 40px rgba(0, 0, 0, 0.18);
    border-radius: 28px 28px 0 0;
    ${(p) =>
      p.$lifted
        ? `position:fixed;top:0;left:0;right:0;bottom:0;border-radius:0;overflow-y:auto;padding:max(2.5rem,calc(env(safe-area-inset-top,0px) + 1.25rem)) 1.5rem max(5rem,calc(env(safe-area-inset-bottom,0px) + 3rem));z-index:500;box-shadow:none;-webkit-overflow-scrolling:touch;background:#fff;`
        : ''}
  }
`
...
const features = [
  {
    Icon: TbTruck,
    title: 'Fast Delivery',
    desc: 'Medicines at your door in hours',
  },
  {
    Icon: TbShieldCheck,
    title: 'Insurance Support',
    desc: 'Direct billing, zero paperwork',
  },
  {
    Icon: TbHeartHandshake,
    title: '24/7 Pharmacy Chat',
    desc: 'Licensed pharmacists always on call',
  },
]
  ...
  return (
    <Page>
      <BrandPanel>
        ...
        >
          Upload prescriptions, manage family profiles, and get medicines
          delivered — all in one place.
        </Text>
      ...
      <FormPanel $lifted={lifted}>
        <MobileHero $lifted={lifted}>
          ...
          >
            Your pharmacy, at your doorstep
          </Text>
        </MobileHero>
        <Card
          $lifted={lifted}
