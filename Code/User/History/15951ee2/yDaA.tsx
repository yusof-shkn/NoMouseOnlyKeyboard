...
const Card = styled.div<{ $lifted?: boolean }>`
  ...
  @media (max-width: 800px) {
    ...
    ${(p) =>
      p.$lifted
        ? `position:fixed;top:0;left:0;right:0;bottom:0;border-radius:0;padding:0;min-height:unset;z-index:500;box-shadow:none;`
        : ''}
  }
`
...
// Inner scroll wrapper — lives inside Card when lifted.
// The Card itself must NOT have overflow-y:auto when position:fixed,
// because that combo breaks native <select> pickers on iOS Safari.
const CardScrollInner = styled.div<{ $lifted?: boolean }>`
  @media (max-width: 800px) {
    ${(p) =>
      p.$lifted
        ? `
  ...
  return (
    <Page>
      ...
      <FormPanel>
        ...
        >
          <CardScrollInner $lifted={lifted}>{formContent}</CardScrollInner>
        </Card>
      </FormPanel>
    </Page>
  )
