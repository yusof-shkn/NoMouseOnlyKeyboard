...
export default function Login(): JSX.Element {
  ...
  const handleFocusOut = useCallback((e: React.FocusEvent) => {
    const tag = (e.target as HTMLElement).tagName.toLowerCase()
    if (tag === 'input' || tag === 'textarea') {
      // Delay un-lifting so a tap on the <select> can complete before the card repositions
      blurTimerRef.current = setTimeout(() => setLifted(false), 350)
    }
  }, [])
  ...
  const formContent = (
    <MobileStack>
      <Box mb={28} className="welcome-header">
        <Text
      ...
      <form onSubmit={handleSubmit}>
        <Stack gap={16} className="form-stack">
          <CountryPhoneInput
            label="Phone Number"
            value={phone}
            onChange={setPhone}
