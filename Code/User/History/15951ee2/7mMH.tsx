...
const Card = styled.div<{ $lifted?: boolean }>`
  ...
  @media (max-width: 800px) {
    ...
    ${(p) =>
      p.$lifted
        ? `position:fixed;top:0;left:0;right:0;bottom:0;border-radius:0;overflow-y:auto;padding:2.5rem 1.5rem 3rem;z-index:500;box-shadow:none;-webkit-overflow-scrolling:touch;`
        : ''}
  }
`
...
export default function Login(): JSX.Element {
  ...
  const handleFocusOut = useCallback((e: React.FocusEvent) => {
    // If focus moved to a child element, don't un-lift
    if (e.currentTarget.contains(e.relatedTarget as Node)) return
    // Short delay so Mantine's portal-based dropdown focus doesn't trigger un-lift
    liftTimerRef.current = setTimeout(() => setLifted(false), 200)
  }, [])
