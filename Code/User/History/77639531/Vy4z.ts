...
export const handleSubmit = async (
  ...
  try {
    ...
    // Add insurance data (handled separately in queries)
    ...
    if (customerData.credit_limit && customerData.credit_limit > 0) {
      if (mode === 'create') {
        customerData.available_credit = customerData.credit_limit
        customerData.current_credit_balance = 0
      } else if (mode === 'edit' && initialValues) {
        const currentBalance = initialValues.current_credit_balance || 0
        customerData.available_credit =
          customerData.credit_limit - currentBalance
      }
    }
