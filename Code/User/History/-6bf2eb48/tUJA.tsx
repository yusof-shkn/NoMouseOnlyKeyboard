...
const DropdownList = styled.ul`
  ...
  box-shadow:
    ...
    0 16px 40px -4px rgba(1, 41, 112, 0.16);
  padding: 6px;
  margin: 0;
  list-style: none;
  max-height: 280px;
...
export function CountryPhoneInput({
  ...
  const recalcPos = useCallback(() => {
    ...
    const dropWidth = Math.max(r.width + 80, 190)
  ...
  // Reposition whenever open or filtered count changes
  useLayoutEffect(() => {
    if (open) {
      recalcPos()
  ...
  // Single click handler — fires once regardless of mouse vs touch
