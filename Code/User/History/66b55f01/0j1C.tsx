...
const SalesReturns = () => {
  ...
  const localHandleApprove = useCallback(
    async (row: SalesReturn) => {
      await handleApprove(row, fetchAllData, companySettings || undefined)
    },
  ...
  const localHandleReject = useCallback(
    async (row: SalesReturn) => {
      await handleReject(row, fetchAllData)
    },
    [fetchAllData],
  )
  ...
  const rowActions = getSalesReturnRowActions(
    {
      onViewDetails: (row) => handleViewDetails(row, navigate),
  ...
  return (
    <PageWrapper>
      <Container
        ...
        px={0}
      >
        {!companySettings?.allow_sales_returns && (
          <Alert
            icon={<IconAlertCircle size={16} />}
            ...
            mb="md"
          >
            Sales returns are currently disabled by company settings. Contact
            your administrator to enable this feature.
          </Alert>
        )}

        {companySettings?.allow_sales_returns &&
          (companySettings.sales_return_days_limit || 0) > 0 && (
            <Alert
            ...
            >
              ...
              {companySettings.sales_return_days_limit || 30} days of purchase.
              {companySettings.require_return_approval &&
                (companySettings.return_approval_threshold || 0) > 0 && (
                  <>
                    {' '}
                    Returns above{' '}
                    {new Intl.NumberFormat('en-UG', {
                      ...
                      companySettings.return_approval_threshold || 0,
                    )}{' '}
                    require approval.
                  </>
                )}
        ...
        <ContentTable
          ...
          onRowClick={(row) => handleViewDetails(row, navigate)}
          selectable={true}
        />
      </Container>
    </PageWrapper>
