...
def run_health_check(verbose: bool = True) -> HealthReport:
    ...
    perms_count = (
        client.table("permissions").select("*", count=CountMethod.exact).execute().count
        or 0
    )

    # Cast data to list[Row] so dict subscript ["id"] is valid
    roles_raw = (
        client.table("roles").select("id").eq("company_id", cid).execute().data or []
    )
    roles_data: list[Row] = cast(list[Row], roles_raw)
    role_ids: list[int] = [int(row["id"]) for row in roles_data]
    ...
    sections = [
        ...
        SectionHealth(
            "role_permissions", rp_count, TEMPLATE_EXPECTED["role_permissions"]
        ),
        SectionHealth(
            "categories", _count("categories"), TEMPLATE_EXPECTED["categories"]
        ),
        SectionHealth("units", _count("units"), TEMPLATE_EXPECTED["units"]),
        SectionHealth("coa", _count("chart_of_accounts"), TEMPLATE_EXPECTED["coa"]),
        SectionHealth(
            "income_cats", _count("income_categories"), TEMPLATE_EXPECTED["income_cats"]
        ),
        SectionHealth(
            "expense_cats",
            _count("expense_categories"),
            TEMPLATE_EXPECTED["expense_cats"],
        ),
    ]
