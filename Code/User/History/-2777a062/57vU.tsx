...
const ChatHead = styled.button<{ $unread: boolean }>`
  position: fixed;
  bottom: 88px;
  right: 20px;
  z-index: 400;
  ...
  box-shadow: 0 4px 20px rgba(21, 179, 224, 0.45);
  display: flex;
  align-items: center;
  justify-content: center;
...
const UnreadDot = styled.span`
  ...
  padding: 0 4px;
`
const ChatDrawer = styled.div<{ $open: boolean }>`
  position: fixed;
  bottom: 0;
  left: 0;
  right: 0;
  z-index: 500;
  height: ${(p: { $open: boolean }) => (p.$open ? '70vh' : '0')};
  ...
  @media (min-width: 900px) {
    ...
    left: auto;
    width: 380px;
    height: ${(p: { $open: boolean }) => (p.$open ? '500px' : '0')};
    border-radius: 20px;
  }
`
...
const ConfirmSpacer = styled.div<{ $show: boolean }>`
  height: ${(p: { $show: boolean }) => (p.$show ? '120px' : '24px')};
  @media (max-width: 899px) {
    height: ${(p: { $show: boolean }) => (p.$show ? '180px' : '24px')};
  }
`
...
export default function OrderTracking() {
  ...
  return (
    <>
      ...
      {/* ── Fixed confirm bar — only mounted when order needs action ── */}
      {order.status === 'awaiting_confirmation' && (
        <ConfirmBar $visible>
          {(order.total_amount ?? 0) === 0 ? (
            <>
