...
export const getIncomeRowActions =
  (handlers: RowActionHandlers, settings?: CompanySettings) =>
  (row: Income, _index: number): RowAction[] => {
    ...
    // System-generated income: only View is allowed
    if (row.is_system) {
      return allActions.filter((a) => a.label === 'View Details')
    }

    // Manual income: filter by what's allowed based on status
    return allActions.filter((a) => {
      if (a.label === 'View Details') return true
      if (a.label === 'Approve') return canPerformAction(row, 'approve').allowed
      if (a.label === 'Post to Journal')
        return canPerformAction(row, 'post').allowed
      if (a.label === 'Edit') return canPerformAction(row, 'edit').allowed
      if (a.label === 'Delete') return canPerformAction(row, 'delete').allowed
      return false
    })
  }
