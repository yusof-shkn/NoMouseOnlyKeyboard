...
// ✅ FIXED: invoiceMeta is now properly passed through handleSubmitPurchaseOrder
//           and forwarded to createPurchaseOrderWithBatches + receiveAllGoodsFromPO
...
// ── Re-export the same InvoiceMeta shape so callers can import from here ──────
...
/**
 * ✅ FIXED: Now includes availableStock and discountAmount
 */
...
/**
 * ✅ FIXED: Validates required fields including new ones
 */
...
/**
 * ✅ FIXED: Updates preserve all required fields
 */
...
export const handleApplyAdditionalCosts = (
  ...
  if (coupon < 0) {
    errors.push('Coupon amount cannot be negative')
  }
  if (discount < 0) {
...
/**
 * ✅ FIXED: Barcode scan now includes all required fields
 */
...
export const handleTogglePurchasePanel = (
  ...
  setIsPanelHidden: (value: boolean) => void,
) => {
  setIsPanelHidden(!isPanelHidden)
}

/**
 * ✅ FIXED: invoiceMeta is now accepted and forwarded to both
 *   createPurchaseOrderWithBatches AND receiveAllGoodsFromPO so that
 *   invoice_number / invoice_date are actually persisted in the DB.
 */
export const handleSubmitPurchaseOrder = async (
  ...
  isAdmin: boolean,
  // ── NEW: invoice metadata forwarded to the query layer ────────────────────
  ...
  try {
    ...
    if (!invoiceNumber) {
      throw new Error('Please enter supplier invoice number')
    }
    ...
    const request = {
      ...
      isAdmin: isAdmin,
      // ── FIXED: pass invoiceMeta so the query layer persists the columns ──
    ...
    const result = await createPurchaseOrderWithBatches(request)

    if (receiveImmediately && isAdmin && result.purchaseOrder?.id) {
      // ── FIXED: forward invoiceMeta so invoice_number is saved on receipt ─
      ...
      onSuccess(result.poNumber, result.purchaseOrder.id, 'received', false)
    } else {
      onSuccess(
        result.poNumber,
...
/**
 * Save as draft - items mapping includes all required fields
 */
...
export const handleReceiveGoods = async (
  ...
  try {
    ...
    const { success } = await receiveAllGoodsFromPO(
      row.id,
      row.company_id,
      userId,
    )
    ...
    if (!success) {
      throw new Error('Failed to receive goods')
    }
