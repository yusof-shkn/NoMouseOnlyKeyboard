...
export default function PharmacyOrderDetail(): JSX.Element {
  ...
  const savePricing = async (): Promise<void> => {
    ...
    // For insured orders: goes to pricing_ready (insurance approval)
    // For cash orders: goes straight to awaiting_confirmation
    const insuredAmount = isCashOrder
      ? insuredMedAmount
      : insuredMedAmount + deliveryFeeCalc
    const cashAmount = cashMedAmount
    const totalAmount = isCashOrder
      ? cashMedAmount + deliveryFeeCalc
      : cashMedAmount
    const newStatus = isCashOrder ? 'awaiting_confirmation' : 'pricing_ready'
    ...
    await supabase.from('order_status_history').insert({
      ...
      notes: isCashOrder
        ? 'Pricing set (cash order)'
        : 'Pricing set — sent to insurance for approval',
    })

    notifications.show({
      title: 'Pricing saved',
      message: isCashOrder
        ? 'Customer notified to review and confirm.'
      ...
      <Box style={{ width: '100%', padding: '2rem' }}>
        ...
        <ContentCard style={{ marginBottom: 24 }}>
          ...
          {pricingSummaryVisible &&
            (() => {
              // Clean additive formula so every line adds/subtracts to Customer Pays:
              //   Subtotal (all meds)
              // + Delivery Fee
              // − Insurance Covers  (insured meds + delivery, only for insured orders)
              // = Customer Pays
              const insuredCoveragePreview = isCashOrder
                ? 0
                : insuredMedAmountPreview +
                  (deliverySkippedLocal ? 0 : DELIVERY_FEE)
              return (
                <>
                  <Divider my="md" />
                    ...
                    >
                      ...
                      <Group gap="xl">
                        ...
                        <Text
                          size="sm"
                          fw={600}
                          c={deliverySkippedLocal ? 'dimmed' : undefined}
                        >
                      ...
                      {insuredCoveragePreview > 0 && (
                        <Group gap="xl">
                          <Text
                            ...
                            c="teal"
                          >
                            Insurance covers (meds + delivery)
                          </Text>
                          <Text
                            size="sm"
                            fw={600}
          ...
          {/* Save pricing button — available in pharmacy_review, pricing_ready, awaiting_confirmation */}
          {canSetPricing && (
            <Group
              justify="flex-end"
              mt="md"
            >
              ...
              >
                {order.status === 'pricing_ready'
                  ...
                  : order.status === 'awaiting_confirmation'
                    ? order.cash_only
                      ? 'Update Price & Re-notify Customer'
                      : 'Update Price & Resubmit to Insurance'
                    : order.cash_only
            ...
            {deliveryStaff.length === 0 ? (
              ...
              <Stack gap="sm">
                ...
                <RiderCard
                  ...
                  style={{
                    borderColor:
                      selectedRider === '__no_rider__' ? '#7C3AED' : undefined,
                    background:
                      selectedRider === '__no_rider__' ? '#f5f3ff' : undefined,
                  }}
                >
                  <RadioDot
                    $selected={selectedRider === '__no_rider__'}
                  ...
                  <Box style={{ flex: 1 }}>
                    ...
                    <Text
                      ...
                      c="dimmed"
                    >
                      Customer collects directly or staff delivers. Skips QR
                      scan — staff can mark as delivered manually.
                    </Text>
                  </Box>
                </RiderCard>
