...
export default function PharmacyOrderDetail(): JSX.Element {
  ...
  const savePricing = async (): Promise<void> => {
    ...
    // Only process medication rows (no delivery rows in table anymore)
    ...
    // Delivery fee is always fixed at DELIVERY_FEE (10,000 UGX)
    // unless order.no_rider is true (in-store pickup / no delivery)
    const subtotal = namedMeds
      .filter((m) => m.in_stock)
      .reduce((s, m) => s + m.unit_price * m.quantity, 0)
    const insuredAmount = namedMeds
      .filter((m) => m.is_insured && m.in_stock)
      .reduce((s, m) => s + m.unit_price * m.quantity, 0)
    const cashTotal = namedMeds
      .filter((m) => !m.is_insured && m.in_stock)
      .reduce((s, m) => s + m.unit_price * m.quantity, 0)
    ...
    const { error: orderErr } = await supabase
      ...
      .update({
        ...
        total_amount: cashTotal + deliveryFeeCalc,
  ...
  const subtotal = meds
    .filter((m) => m.in_stock)
    .reduce((s, m) => s + m.unit_price * m.quantity, 0)
  const insuredAmount = meds
    .filter((m) => m.is_insured && m.in_stock)
    .reduce((s, m) => s + m.unit_price * m.quantity, 0)
  const cashTotal = meds
    .filter((m) => !m.is_insured && m.in_stock)
    .reduce((s, m) => s + m.unit_price * m.quantity, 0)
                  ...
                  {insuredAmount > 0 && (
                    <Group gap="xl">
                      ...
                      <Text
                        ...
                        c="teal"
                      >
                        − UGX {insuredAmount.toLocaleString()}
                      </Text>
                    </Group>
                  )}
                  <Group gap="xl">
          ...
          >
            ...
            >
              ...
              <Box>
                <Text
                  ...
                  c={SavedColors.TextColor}
                >
                  OTP Fallback — Customer Backup Code
                </Text>
                <Text
                  size="xs"
                  c="dimmed"
                  mt={2}
                >
                  If the rider cannot scan the customer&apos;s QR code, share
                  this OTP with the customer — they will read it to the rider
                  over the phone to confirm delivery.
                </Text>
              </Box>
            </Group>
            <Alert
            ...
            >
              <Text
                ...
                fw={600}
              >
                Share this OTP with the customer, not the rider
              </Text>
              <Text
                size="xs"
                mt={2}
                c="dimmed"
              >
                The customer reads their OTP to the rider over the phone to
                confirm delivery.
              </Text>
            </Alert>
