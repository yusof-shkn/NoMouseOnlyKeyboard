  ...
  const savePricing = async (): Promise<void> => {
    ...
    if (wasInsuranceApproved) {
      ...
      const changedInsuredMed = namedMeds.some((m) => {
        ...
        // Was insured before or is insured now
        if (orig.is_insured || m.is_insured) {
          // Coverage toggle always needs re-approval
          if (orig.is_insured !== m.is_insured) return true
          // Unit price change always needs re-approval
          if (orig.unit_price !== m.unit_price) return true
          // Quantity change: only needs re-approval if the new quantity is GREATER than
          // the approved_qty (the pharmacist is prescribing more than insurance approved).
          // If new qty ≤ approved_qty, insurance approval still covers everything — no re-approval.
          if (orig.quantity !== m.quantity) {
            const approvedQty = orig.approved_qty ?? orig.quantity
            // New quantity exceeds what insurance approved → must go back to insurance
            if (m.quantity > approvedQty) return true
            // New quantity ≤ approved_qty → insurance approval still valid, no re-approval
            return false
          }
        }
        return false
    ...
    for (const med of namedMeds) {
      if (med.isNew) {
        ...
        // When quantity is reduced below approved_qty, clamp approved_qty to the new quantity.
        // This keeps the DB consistent without needing insurance re-approval.
        ...
        const clampedApprovedQty =
          existingApprovedQty != null
            ...
            : null
        // If clamped approved_qty equals the new quantity, status becomes 'approved';
        // if it's less, it remains 'partial'. Both are fine — no re-approval needed.
