...
// ============== GLOBAL STATS (uses DB aggregate — not page-limited) ==============

/**
 * ✅ Fetch stats across ALL sales using the get_sales_stats RPC.
 * This is NOT limited by pagination, so stat cards always show true totals.
 */
export const fetchGlobalSalesStats = async (
  companyId: number,
  storeId?: number,
): Promise<SalesStats> => {
  try {
    ...
    if (error || !data) {
      console.warn('get_sales_stats RPC failed, falling back to 0s:', error)
      return {
        total: 0,
        pending: 0,
        completed: 0,
    ...
    return {
      ...
      pending: data.pendingSales || 0,
      completed: data.completedSales || 0,
      cancelled: data.cancelledSales || 0,
      returned: 0, // not in RPC — kept for compatibility
      pendingReturn: 0, // not in RPC — kept for compatibility
      totalAmount: data.totalRevenue || 0,
      totalDiscount: 0,
      totalTax: 0,
    }
  } catch (err) {
    console.error('fetchGlobalSalesStats error:', err)
    return {
      total: 0,
      pending: 0,
      completed: 0,
...
export const canDeleteSale = (
  ...
  // Completed sales CAN be deleted by admin — triggers will reverse stock, income, credit
  if (sale.sale_status === 'completed') {
    return { allowed: true }
  }
...
export const calculateStats = (salesData: any[]): SalesStats => {
  const pending =
    salesData.filter((sale) => sale.sale_status === 'draft').length || 0
  const completed =
