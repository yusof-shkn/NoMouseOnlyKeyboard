...
const Page = styled.div`
  min-height: 100vh;
  min-height: 100dvh; /* shrinks when soft keyboard opens on mobile */
  display: flex;
  align-items: flex-start;
  justify-content: center;
  padding: 2rem 1.5rem 2.5rem;
  position: relative;
  /* overflow-x only — never clip vertical scroll */
  overflow-x: clip;
  overflow-y: auto;
  background: #f0f4fa;
  &::before {
...
const Card = styled.div`
  ...
  @media (max-width: 640px) {
    ...
    box-shadow: none;
    border: none;
    /* Extra bottom padding ensures the submit button is always above the keyboard */
    padding: 1rem 1.25rem env(safe-area-inset-bottom, 1.5rem);
    padding-bottom: max(env(safe-area-inset-bottom, 0px) + 24px, 80px);
    width: 100%;
    margin-top: 0;
  }
`
...
const PlainInputErr = styled(PlainInput)<{ $err?: boolean }>`
  border-color: ${(p) => (p.$err ? '#ef4444' : '#e4e9f0')};
  &:focus {
    border-color: ${(p) => (p.$err ? '#ef4444' : '#15b3e0')};
    box-shadow: 0 0 0 3px
      ${(p) => (p.$err ? 'rgba(239,68,68,0.12)' : 'rgba(21,179,224,0.12)')};
  }
`
...
export default function Register(): JSX.Element {
  ...
  const e164 = phone // react-phone-number-input returns E.164 directly
  ...
  const handleSignUp = async (ev: React.FormEvent) => {
    ...
    if (!phone || phone.length < 8) errs.phone = 'Enter a valid phone number'
    ...
    if (!agreed)
      errs.agreed = 'You must accept the Terms and Privacy Policy to continue'
            ...
            <FormStack gap={10}>
              <Box>
                ...
                {fieldErrors.fullName && (
                  <ErrMsg>{fieldErrors.fullName}</ErrMsg>
                )}
              </Box>
              ...
              <Box>
                ...
                />
                {fieldErrors.email && <ErrMsg>{fieldErrors.email}</ErrMsg>}
              </Box>
              ...
              <Box>
                ...
                {fieldErrors.password && (
                  <ErrMsg>{fieldErrors.password}</ErrMsg>
                )}
              </Box>
              ...
              <Box>
                ...
                {fieldErrors.confirmPwd && (
                  <ErrMsg>{fieldErrors.confirmPwd}</ErrMsg>
                )}
              </Box>
              ...
              <Box>
                <Checkbox
                  ...
                  color={fieldErrors.agreed ? 'red' : 'lotusCyan'}
                  radius={4}
                  label={
                    <Text
                      style={{
                ...
                />
                {fieldErrors.agreed && (
                  <ErrMsg style={{ marginTop: 6 }}>{fieldErrors.agreed}</ErrMsg>
                )}
              </Box>
