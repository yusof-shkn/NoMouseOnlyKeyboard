...
export const getSalesColumns = (
  ...
  const allCols = [
    ...
    },
    {
      ...
      render: (row: any) => {
        ...
        const profile = row.profiles as { first_name?: string; last_name?: string; username?: string } | null
        const name = profile?.first_name && profile?.last_name
          ? `${profile.first_name} ${profile.last_name}`.trim()
          : profile?.username || null
        return name ? (
          <Group gap={6} wrap="nowrap">
            <IconUser size={14} color={isDark ? theme.colors.gray[5] : theme.colors.gray[6]} style={{ flexShrink: 0 }} />
            <Text size="sm" c={isDark ? 'gray.3' : 'gray.8'}>
              {name}
            </Text>
          </Group>
        ) : (
  ...
  const TABLE_COLS = ['sale_number', 'customer_name', 'sale_date', 'store_name', 'sale_status', 'payment_method', 'total_amount', 'payment_status', 'profiles']
