...
import { useDeliveryLoginForm } from '../../../features/delivery/login-form/model/useDeliveryLoginForm'
...
const features = [
  {
    ...
    desc: 'Accept or decline new delivery assignments instantly',
  },
  {
    Icon: TbMapPin,
    title: 'Live GPS Tracking',
    desc: 'Your location is shared with customers in real time',
  },
  {
    Icon: TbQrcode,
    title: 'QR Confirmation',
    desc: 'Scan customer QR to confirm and close a delivery',
  },
]

export default function DeliveryLogin(): JSX.Element {
  const navigate = useNavigate()
  const { user, staffInfo, loading } = useAuth()
  const { control, handleSubmit, onSubmit, isSubmitting } =
    useDeliveryLoginForm()

  useEffect(() => {
    if (loading) return
    if (user && staffInfo?.role === 'delivery') navigate('/delivery/dashboard')
  }, [user, staffInfo, loading, navigate])
  ...
  return (
    <Page>
      <BrandPanel>
        ...
        >
          Accept delivery requests, share your live location with customers, and
          confirm deliveries with a quick QR scan.
        </Text>
            ...
            <Stack gap={16}>
              <InputWrap>
                <FormInput
                  ...
                  control={control}
                  label="Staff Email"
                  placeholder="rider@medideliver.com"
              ...
              >
                ...
                >
                  🔒 This portal is for delivery riders only. Accounts are
              ...
              >
                {isSubmitting ? 'Signing in…' : 'Sign In to Delivery Portal'}
              </PrimaryBtn>
            </Stack>
          </form>
        </Card>
