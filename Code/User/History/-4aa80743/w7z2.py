...
def prompt_format_company(company_id: int) -> bool:
    ...
    print("=" * 60)
    print(f"  This will DELETE all existing seeded data for company_id={company_id}:")
    for t in _TABLES_TO_CLEAR:
        print(f"    • {t}")
    print()
    answer = (
        input("  Would you like to format this company? [yes/NO]: ").strip().lower()
    )
...
def seed_sales(company_id: int) -> None:
    ...
    for idx, (
        ...
        customer_id: int | None = (
            customer_ids[idx % len(customer_ids)] if customer_ids else None
        )
