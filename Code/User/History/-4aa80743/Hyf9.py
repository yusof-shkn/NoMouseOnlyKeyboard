...
- seed_income_categories: added `is_system=True`, `is_default=True` on "Medicine Sales"
  so `fn_create_income_from_sale` trigger resolves category_id (not null violation fix)
- seed_expense_categories: added `is_system=True`, `is_default=True` on "Cost of Goods Sold"
  so `fn_create_expense_from_po` trigger resolves category_id (not null violation fix)
- seed_inventory_batches: fixed product lookup to use NDA products (company_id=1) since
  non-template companies share the global NDA catalogue; own products fallback still works
- seed_purchase_orders: removed non-existent column `payment_method` from row dict
  (purchase_orders.payment_method is varchar with default 'cash', no issue, kept as-is)
- seed_expenses (direct seeder): expense_number format aligned to YYYYMMDD-SEED-NNN to avoid
  collisions with trigger-generated expense numbers; also added `deleted_at=None` guard
- seed_income (direct seeder): same income_number format alignment
- Pipeline: categories/units seeded before products; expense/income cats seeded before
  purchase_orders/sales to guarantee trigger FK resolution on first run
- NEW: expense_number null fix — expenses seeder now always generates a non-null number
- NEW: pre-seed format prompt — asks user to confirm before deleting company data
- NEW: type annotations cleaned up (removed unnecessary cast/Any usage)
...
def _gen_expense_number(company_id: int, idx: int) -> str:
    """Generate a stable, non-null expense number that won't collide with trigger-generated ones."""
    today_str = datetime.utcnow().strftime("%Y%m%d")
    return f"{today_str}-SEED-{company_id:02d}-{idx + 1:03d}"


def _gen_income_number(company_id: int, idx: int) -> str:
    """Generate a stable, non-null income number that won't collide with trigger-generated ones."""
    today_str = datetime.utcnow().strftime("%Y%m%d")
    return f"{today_str}-SEED-{company_id:02d}-{idx + 1:03d}"


# ─────────────────────────────────────────────────────────────────────────────
# PRE-SEED: Format (delete) prompt
# ─────────────────────────────────────────────────────────────────────────────
...
def prompt_format_company(company_id: int) -> bool:
    """
    Ask the user whether to wipe all seeded data for this company before re-seeding.
    Returns True if the user confirmed deletion (and deletion was performed),
    False if the user skipped.

    Usage:
        deleted = prompt_format_company(company_id)
    """
    print()
    print("=" * 60)
    ...
    print("=" * 60)
    print(
        f"  This will DELETE all existing seeded data for company_id={company_id}:"
    )
    for t in _TABLES_TO_CLEAR:
        print(f"    • {t}")
    print()
    answer = input("  Would you like to format this company? [yes/NO]: ").strip().lower()
    if answer not in ("yes", "y"):
        print("  → Skipping format. Seeding on top of existing data (idempotent).")
        print()
        return False

    print(f"\n  🗑  Deleting data for company_id={company_id} …")
    client = get_client()
...
# ─────────────────────────────────────────────────────────────────────────────
_ROLES: list[tuple[str, str, int, str]] = [
...
# ─────────────────────────────────────────────────────────────────────────────
_CORE_PERMS: list[tuple[int, str, str, str, str, str]] = [
...
_EXTRA_MODULES: list[tuple[str, str, list[str]]] = [
...
def seed_role_permissions(company_id: int) -> None:
    ...
    role_id_map: dict[str, int] = {r["role_name"]: r["id"] for r in company_roles}
    ...
    role_predicates: list[tuple[str, Callable[[dict], bool]]] = [
    ...
    for role_name, pred in role_predicates:
        role_id: int | None = role_id_map.get(role_name)
        if not role_id:
            log_warn(f"Role '{role_name}' not found for company {company_id}, skipping.")
            continue
        for p in all_perms:
            if pred(p):
...
def seed_units(company_id: int) -> None:
    ...
    base_units: list[dict] = [
        {"name": "Tablet", "short_code": "TAB", "type": "base", "base_unit_id": None, "conversion_factor": 1},
        {"name": "Capsule", "short_code": "CAP", "type": "base", "base_unit_id": None, "conversion_factor": 1},
        {"name": "Bottle", "short_code": "BTL", "type": "base", "base_unit_id": None, "conversion_factor": 1},
        {"name": "Vial", "short_code": "VIAL", "type": "base", "base_unit_id": None, "conversion_factor": 1},
        {"name": "Tube", "short_code": "TUBE", "type": "base", "base_unit_id": None, "conversion_factor": 1},
        {"name": "Milliliter", "short_code": "ML", "type": "base", "base_unit_id": None, "conversion_factor": 1},
        {"name": "Milligram", "short_code": "MG", "type": "base", "base_unit_id": None, "conversion_factor": 1},
    ]
    ...
    derived_units: list[dict] = [
        {"name": "Strip", "short_code": "STRIP", "type": "derived", "base_short_code": "TAB", "conversion_factor": 10},
        {"name": "Sachet", "short_code": "SACH", "type": "derived", "base_short_code": "ML", "conversion_factor": 5},
    ]
    for u in derived_units:
        base_id: int | None = unit_id_map.get(u["base_short_code"])
        existing = (
            client.table("units")
            .select("id")
            .eq("company_id", company_id)
    ...
    strip_id: int | None = unit_id_map.get("STRIP")
    existing = (
        client.table("units")
        .select("id")
        .eq("company_id", company_id)
...
# ─────────────────────────────────────────────────────────────────────────────
_COA: list[tuple[str, str, str, str]] = [
...
def seed_chart_of_accounts(company_id: int) -> None:
    ...
    for code, name, atype, subtype in _COA:
        ...
        if existing:
            client.table("chart_of_accounts").update(row).eq("id", existing[0]["id"]).execute()
        else:
...
# 8. Income categories
# FIX: upsert on (company_id, category_name) not bare id
# FIX: is_system=True + is_default=True on "Medicine Sales" so that
#      fn_create_income_from_sale trigger resolves category_id (not null violation fix)
# NOTE: ALL categories get is_system=True so the trigger's fallback query
#       (which filters is_system=true) always finds at least one row.
# ─────────────────────────────────────────────────────────────────────────────

# (name, is_default)  — only one should be default
_INCOME_CATS: list[tuple[str, bool]] = [
    ("Income", False),
    ("Medicine Sales", True),          # ← DEFAULT: trigger's primary query hits this
    ("Surgical & Medical Supplies", False),
    ("Baby & Mother Care", False),
    ("Cosmetics & Beauty", False),
    ("Nutrition & Supplements", False),
    ("Medical Devices", False),
    ("Laboratory Services", False),
    ("Consultation Fees", False),
    ("Vaccine & Immunization Services", False),
    ("Online/Telepharmacy Sales", False),
    ("Insurance Reimbursements", False),
    ("Government Subsidies/Grants", False),
    ("Other Income", False),
    ("Prescription Drugs", False),
    ("Over-the-Counter (OTC)", False),
]


def seed_income_categories(company_id: int) -> None:
    client = get_client()
    for i, (name, is_default) in enumerate(_INCOME_CATS, 1):
        existing: list[dict] = (
        ...
        row: dict = {
            ...
            "is_active": True,
            # ALL rows get is_system=True so the trigger's fallback query
            # (filters on is_system=true) always resolves at least one category_id.
            # is_default=True on "Medicine Sales" makes it the primary resolution target.
            "is_system": True,
            "is_default": is_default,
        }
        if existing:
            client.table("income_categories").update(row).eq("id", existing[0]["id"]).execute()
        else:
...
# FIX: upsert on (company_id, category_name) not bare id
# FIX: is_system=True + is_default=True on "Cost of Goods Sold" so that
#      fn_create_expense_from_po trigger's last-resort fallback resolves category_id.
# NOTE: ALL categories get is_system=True so the trigger's fallback query
#       (which filters is_system=true) always finds at least one row.
# ─────────────────────────────────────────────────────────────────────────────

# (name, is_default)
_EXPENSE_CATS: list[tuple[str, bool]] = [
    ("Expenses", False),
    ("Cost of Goods Sold", True),       # ← DEFAULT: trigger fallback finds this first
    ("Salaries & Wages", False),
    ("Rent & Utilities", False),
    ("Licenses & Regulatory Fees", False),
    ("Marketing & Advertising", False),
    ("Professional Fees", False),
    ("Insurance", False),
    ("Bank Charges & Interest", False),
    ("Repairs & Maintenance", False),
    ("Office Supplies", False),
    ("Transportation & Delivery", False),
    ("Taxes & Duties", False),
    ("Training & Staff Development", False),
    ("Depreciation", False),
    ("Inventory Loss/Shrinkage", False),
    ("Waste Disposal", False),
    ("Software & IT Subscriptions", False),
    ("Patient Assistance Programs", False),
    ("Other Expenses", False),
    ("Medicine Purchases", False),
    ("Supplies Purchases", False),
    ("Pharmacist Salaries", False),
    ("Support Staff Wages", False),
]


def seed_expense_categories(company_id: int) -> None:
    client = get_client()
    for i, (name, is_default) in enumerate(_EXPENSE_CATS, 1):
        existing: list[dict] = (
        ...
        row: dict = {
            ...
            "is_active": True,
            # ALL rows get is_system=True so the trigger's last-resort fallback
            # (filters on is_active=true AND deleted_at IS NULL) always resolves
            # a category_id. is_default marks the preferred auto-created category.
            "is_system": True,
            "is_default": is_default,
        }
        if existing:
            client.table("expense_categories").update(row).eq("id", existing[0]["id"]).execute()
        else:
            client.table("expense_categories").insert(row).execute()
    log_ok(f"Expense categories ensured ({len(_EXPENSE_CATS)}) for company {company_id}.")
...
_PRODUCTS_TEMPLATE: list[tuple[str, str, str, str, bool, bool, int, int, str, str]] = [
    ("Paracetamol 500mg Tabs", "Paracetamol", "Tablet", "500mg", False, False, 800, 1500, "TABLETS", "TAB"),
    ("Amoxicillin 250mg Caps", "Amoxicillin", "Capsule", "250mg", True, False, 2500, 4500, "CAPSULES", "CAP"),
    ("Metformin 500mg Tabs", "Metformin", "Tablet", "500mg", True, False, 1200, 2200, "TABLETS", "TAB"),
    ("ORS Sachet", "Oral Rehydration Salts", "Sachet", "Standard", False, False, 300, 600, "SYRUPS", "SACH"),
    ("Ibuprofen 400mg Tabs", "Ibuprofen", "Tablet", "400mg", False, False, 900, 1800, "TABLETS", "TAB"),
    ("Amoxicillin 125mg Syrup", "Amoxicillin", "Syrup", "125mg/5ml", True, False, 5000, 8500, "SYRUPS", "BTL"),
    ("Ciprofloxacin 500mg Tabs", "Ciprofloxacin", "Tablet", "500mg", True, False, 3500, 6000, "TABLETS", "TAB"),
    ("Omeprazole 20mg Caps", "Omeprazole", "Capsule", "20mg", True, False, 2000, 3500, "CAPSULES", "CAP"),
    ("Diclofenac 50mg Tabs", "Diclofenac", "Tablet", "50mg", True, False, 1100, 2000, "TABLETS", "TAB"),
    ("Dettol 750ml Antiseptic", "Chloroxylenol", "Liquid", "4.8%", False, False, 12000, 18000, "DROPS", "BTL"),
    ("Morphine 10mg Tabs", "Morphine Sulfate", "Tablet", "10mg", True, True, 8000, 14000, "TABLETS", "TAB"),
    ("Insulin Glargine 100U/ml", "Insulin Glargine", "Injection", "100U/ml", True, True, 45000, 75000, "INJECTABLES", "VIAL"),
    ("Salbutamol Inhaler", "Salbutamol", "Inhaler", "100mcg", True, False, 15000, 25000, "DROPS", "BTL"),
    ("Multivitamin Tabs", "Multivitamins", "Tablet", "Standard", False, False, 500, 1000, "TABLETS", "TAB"),
    ("Zinc 20mg Tabs", "Zinc Sulfate", "Tablet", "20mg", False, False, 400, 800, "TABLETS", "TAB"),
]
...
def seed_products(company_id: int) -> None:
    ...
    cat_map: dict[str, int] = {c["category_code"]: c["id"] for c in cats}
    ...
    unit_map: dict[str, int] = {u["short_code"]: u["id"] for u in units}

    inserted = 0
    for i, (
        name, generic, dosage, strength, rx, controlled, cost, price, cat_code, unit_code
    ...
    log_ok(f"Products ensured ({len(_PRODUCTS_TEMPLATE)}, {inserted} new) for company {company_id}.")
...
def seed_inventory_batches(company_id: int) -> None:
    ...
    # Primary: own products for this company
    products: list[dict] = (
        client.table("products")
    ...
    # If no own products exist (NDA-only company), use NDA product IDs already
    # referenced by existing batches so we stay idempotent.
    if not products:
        existing_batch_product_ids: list[int] = [
            row["product_id"]
            for row in (
        ...
        if existing_batch_product_ids:
            log_ok(
                f"No own products for company {company_id}; "
                f"{len(set(existing_batch_product_ids))} NDA products already have batches — skipping."
            )
            return

        nda_products: list[dict] = (
            client.table("products")
            .select("id,product_code,standard_cost,standard_price,is_restricted")
            .eq("company_id", 1)
            .limit(13)
        ...
        if not nda_products:
            log_warn("No products found at all — skipping inventory batches.")
            return
        products = nda_products
    ...
    if not stores:
        log_warn("No stores found — skipping inventory batches.")
        return

    supplier_ids: list[int] = [s["id"] for s in suppliers]
    store_ids: list[int] = [s["id"] for s in stores]
    inserted = 0

    for prod in products:
        for j, store_id in enumerate(store_ids):
            batch_num = f"BCH-P{prod['id']}-S{store_id:02d}-001"
            existing: list[dict] = (
                client.table("product_batches")
                .select("id")
                .eq("company_id", company_id)
            ...
            row: dict = {
                ...
                "supplier_id": supplier_ids[j % len(supplier_ids)] if supplier_ids else None,
                "is_active": True,
                "is_expired": expiry_offset < 0,
                "is_restricted": prod.get("is_restricted", False),
                "created_at": _now(),
                "updated_at": _now(),
            }
            if existing:
                client.table("product_batches").update(row).eq("id", existing[0]["id"]).execute()
            else:
...
# NOTE: income_categories with is_system=True must be seeded BEFORE this runs
#       so that fn_create_income_from_sale trigger can resolve category_id.
# ─────────────────────────────────────────────────────────────────────────────
def seed_sales(company_id: int) -> None:
    ...
    if not stores:
        log_warn("Missing stores — skipping sales.")
        return

    store_ids: list[int] = [s["id"] for s in stores]
    customer_ids: list[int] = [c["id"] for c in customers]
    inserted = 0

    # (days_ago, payment_method, payment_status, sale_status, is_restricted)
    # FIX: "partial" → "partially_paid" (invalid enum value)
    sale_templates: list[tuple[int, str, str, str, bool]] = [
    ...
    for idx, (days_ago, pay_method, pay_status, sale_status, is_restricted) in enumerate(sale_templates):
        store_id: int = store_ids[idx % len(store_ids)]
        customer_id: int | None = customer_ids[idx % len(customer_ids)] if customer_ids else None
        ...
        row: dict = {
            ...
            "tax_amount": 0,
            "total_amount": total,
            "amount_paid": total if pay_status == "paid"
    ...
    log_ok(f"Sales ensured ({len(sale_templates)}, {inserted} new) for company {company_id}.")


# ─────────────────────────────────────────────────────────────────────────────
# 13. Purchase orders + returns
# NOTE: expense_categories with is_system=True must be seeded BEFORE this runs
#       so that fn_create_expense_from_po trigger can resolve category_id.
# ─────────────────────────────────────────────────────────────────────────────
def seed_purchase_orders(company_id: int) -> None:
    ...
    po_templates: list[tuple[int, str, bool]] = [
        (5,  "received", False),
        (10, "received", True),
        (15, "approved", False),
        (20, "pending",  True),
    ...
    for idx, (days_ago, status, is_restricted) in enumerate(po_templates):
        ...
        store_id: int = store_ids[idx % len(store_ids)]
        supplier_id: int = supplier_ids[idx % len(supplier_ids)]
        ...
        if existing:
            client.table("purchase_orders").update(row).eq("id", existing[0]["id"]).execute()
        else:
            client.table("purchase_orders").insert(row).execute()
            inserted += 1

    log_ok(f"Purchase orders ensured ({len(po_templates)}, {inserted} new) for company {company_id}.")
...
def seed_purchase_returns(company_id: int) -> None:
    ...
    ret_templates: list[tuple[int, str, bool]] = [
        (0, "approved",  False),
        (1, "pending",   True),
        (2, "completed", False),
        (3, "approved",  True),
    ...
    for idx, (po_idx, status, is_restricted) in enumerate(ret_templates):
        ...
        if existing:
            client.table("purchase_returns").update(row).eq("id", existing[0]["id"]).execute()
        else:
...
def seed_sales_returns(company_id: int) -> None:
    ...
    for idx, (sale_idx, status, is_restricted) in enumerate(ret_templates):
        ...
        if existing:
            client.table("sales_returns").update(row).eq("id", existing[0]["id"]).execute()
        else:
...
# FIX: expense_number always generated (never null) — format YYYYMMDD-SEED-CC-NNN
#      This avoids collision with trigger-generated numbers (YYYYMMDD-NNNNNN)
#      while remaining stable/idempotent across re-runs.
# ─────────────────────────────────────────────────────────────────────────────
def seed_expenses(company_id: int) -> None:
    ...
    store_ids: list[int] = [s["id"] for s in stores]
    cat_ids: list[int] = [c["id"] for c in cats]
    inserted = 0

    # (days_ago, amount, payment_method, status, is_restricted)
    exp_templates: list[tuple[int, int, str, str, bool]] = [
    ...
    for idx, (days_ago, amount, pay_method, status, is_restricted) in enumerate(exp_templates):
        # FIX: always generate a non-null expense number with SEED prefix
        exp_num = _gen_expense_number(company_id, idx)
        store_id: int = store_ids[idx % len(store_ids)]
        cat_id: int = cat_ids[idx % len(cat_ids)]
        ...
        row: dict = {
            ...
            "store_id": store_id,
            "category_id": cat_id,
            "expense_number": exp_num,               # FIX: always non-null
    ...
    log_ok(f"Expenses ensured ({len(exp_templates)}, {inserted} new) for company {company_id}.")


# ─────────────────────────────────────────────────────────────────────────────
# 16. Income
# FIX: removed non-existent columns `discount_percentage` and `discount_amount`
# FIX: income_number always generated (never null) — format YYYYMMDD-SEED-CC-NNN
# ─────────────────────────────────────────────────────────────────────────────
def seed_income(company_id: int) -> None:
    ...
    # (days_ago, amount, payment_method, status, is_restricted)
    inc_templates: list[tuple[int, int, str, str, bool]] = [
    ...
    for idx, (days_ago, amount, pay_method, status, is_restricted) in enumerate(inc_templates):
        # FIX: always generate a non-null income number with SEED prefix
        inc_num = _gen_income_number(company_id, idx)
        store_id: int = store_ids[idx % len(store_ids)]
        cat_id: int = cat_ids[idx % len(cat_ids)]
        ...
        row: dict = {
            ...
            "store_id": store_id,
            "category_id": cat_id,
            "income_number": inc_num,                # FIX: always non-null
    ...
    log_ok(f"Income ensured ({len(inc_templates)}, {inserted} new) for company {company_id}.")
...
def make_pipeline(company_id: int) -> list[tuple[str, Callable[[], None]]]:
    return [
        ...
        # IMPORTANT: income_cats and expense_cats MUST run before purchase_orders and sales.
        # The DB triggers fn_create_expense_from_po and fn_create_income_from_sale
        # look up category_id at INSERT time. On first run, if categories don't exist
        # the trigger sets category_id=NULL → NOT NULL constraint violation.
...
def run_pipeline(company_id: int, *, skip_format_prompt: bool = False) -> None:
    """
    Run the full seed pipeline for a given company.

    Args:
        company_id: Target company ID.
        skip_format_prompt: If True, skip the interactive format/delete prompt
                            (useful for automated tests or CI).
    """
    if not skip_format_prompt:
        prompt_format_company(company_id)

    pipeline = make_pipeline(company_id)
    total = len(pipeline)
    for step_idx, (name, fn) in enumerate(pipeline, 1):
        print(f"  [{step_idx}/{total}] {name} …", end=" ", flush=True)
        try:
            fn()
        except Exception as exc:  # noqa: BLE001
