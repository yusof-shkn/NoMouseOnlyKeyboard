...
- seed_expenses (direct seeder): expense_number format aligned to YYYYMMDD-NNNNNN to avoid
...
# 8. Income categories
# FIX: upsert on (company_id, category_name) not bare id
# FIX: added is_system=True and is_default=True on "Medicine Sales" so that
#      fn_create_income_from_sale trigger can resolve category_id (not-null fix)
# ─────────────────────────────────────────────────────────────────────────────
...
def seed_income_categories(company_id: int) -> None:
    ...
    for i, (name, is_default) in enumerate(_INCOME_CATS, 1):
        ...
        row = {
            ...
            "is_active": True,
            # FIX: is_system + is_default required for trigger fn_create_income_from_sale
            # to resolve category_id; without is_default=True on at least one row,
            # the trigger's first SELECT returns NULL → NOT NULL violation on income.category_id
            "is_system": True,
...
# FIX: upsert on (company_id, category_name) not bare id
# FIX: added is_system=True and is_default=True on "Cost of Goods Sold" so that
#      fn_create_expense_from_po trigger's last-resort fallback resolves category_id.
#      The trigger looks for is_system=true with specific codes first; those don't exist
#      in this seed so we ensure the fallback (any active category for company) always
#      finds a row by also setting is_system=True to match the query filter.
# ─────────────────────────────────────────────────────────────────────────────
...
def seed_expense_categories(company_id: int) -> None:
    ...
    for i, (name, is_default) in enumerate(_EXPENSE_CATS, 1):
        ...
        row = {
            ...
            "is_active": True,
            # FIX: is_system=True makes rows visible to the trigger's last-resort
            # fallback which filters on is_active=true AND deleted_at IS NULL.
            # is_default marks the preferred category for auto-created expense records.
            "is_system": True,
            "is_default": is_default,
        }
        if existing:
...
#
# FIX: For companies using the global NDA catalogue (products owned by company_id=1),
#      own-company products (company_id = company_id) are seeded by seed_products above.
#      We query BOTH sets: first own products, then fall back to NDA products that already
#      have batches for this company (to avoid re-seeding on idempotent runs).
#      Batch number is keyed on product_id + store_id so it is stable across runs.
# ─────────────────────────────────────────────────────────────────────────────
def seed_inventory_batches(company_id: int) -> None:
    ...
    # FIX: If no own products exist (NDA-only company), use the NDA product IDs
    # already referenced by existing batches for this company so we stay idempotent.
    if not products:
        existing_batch_product_ids = [
            row["product_id"]
            for row in (
        ...
        # No own products AND no existing batches: load a small NDA product sample
        nda_products = (
            client.table("products")
            .select("id,product_code,standard_cost,standard_price,is_restricted")
            .eq("company_id", 1)
    ...
    for prod in products:
        for j, store_id in enumerate(store_ids):
            # FIX: stable batch number keyed on product_id + store_id (not product_code slice)
            # to avoid collisions when product codes are long NDA codes
...
def seed_purchase_orders(company_id: int) -> None:
    ...
    for idx, (days_ago, status, is_restricted) in enumerate(po_templates):
        ...
        row = {
            ...
            "total_amount": total,
            "payment_terms": "net30",
            # FIX: purchase_orders.payment_method is varchar(default 'cash'), valid as-is
            "payment_method": "cash",
            "status": status,
...
#     FIX: expense_number format changed to YYYYMMDD-SEED-NNN to avoid collision
#          with trigger-generated numbers (YYYYMMDD-NNNNNN) while remaining
#          stable/idempotent across re-runs
# ─────────────────────────────────────────────────────────────────────────────
def seed_expenses(company_id: int) -> None:
    ...
    for idx, (days_ago, amount, pay_method, status, is_restricted) in enumerate(
        ...
        # FIX: prefix "SEED" distinguishes seeder-created numbers from trigger-created
        # ones (YYYYMMDD-000001 format) and keeps them stable/idempotent
        exp_num = f"{today_str}-SEED-{company_id:02d}-{idx + 1:03d}"
        store_id = store_ids[idx % len(store_ids)]
...
# 16. Income
#     FIX: removed non-existent columns `discount_percentage` and `discount_amount`
#     FIX: income_number format changed to YYYYMMDD-SEED-NNN (same reason as expenses)
# ─────────────────────────────────────────────────────────────────────────────
def seed_income(company_id: int) -> None:
    ...
    for idx, (days_ago, amount, pay_method, status, is_restricted) in enumerate(
        ...
        # FIX: prefix "SEED" distinguishes seeder-created numbers from trigger-created ones
        inc_num = f"{today_str}-SEED-{company_id:02d}-{idx + 1:03d}"
        store_id = store_ids[idx % len(store_ids)]
        cat_id = cat_ids[idx % len(cat_ids)]
...
def make_pipeline(company_id: int) -> list[tuple[str, Callable[[], None]]]:
    return [
        ...
        # FIX: income_cats and expense_cats MUST run before purchase_orders and sales
        # because the DB triggers fn_create_expense_from_po and fn_create_income_from_sale
        # look up category_id at INSERT time. On first run, if categories don't exist yet
        # the trigger sets category_id=NULL which violates the NOT NULL constraint.
