...
UNITS STRATEGY
--------------
Products live in the global template (company_id = 1).
Units are company-scoped (each company gets its own copy via the
copy_all_template_data_to_new_company trigger).

To bridge this gap we store a `unit_code` (e.g. "TAB", "CAP", "VIAL")
directly on every product row.  The scraper maps dosage_form → unit_code
using DOSAGE_FORM_TO_UNIT_CODE below, and writes it on every INSERT/UPDATE.

At query time, views and the frontend resolve the actual unit row via:
  resolve_unit_id(company_id, unit_code)   -- DB function
  or: JOIN units ON short_code = product.unit_code AND company_id = ?
"""
...
# ── Unit-code mapping ─────────────────────────────────────────────────────────
# Maps every known NDA dosage_form keyword → unit short_code.
# Priority: first match wins (most-specific phrases listed first).
# Used by the scraper when inserting/updating products.
#
# Short codes that exist in the units table:
#   TAB   – Tablet       (solid, counted individually)
#   CAP   – Capsule      (solid, counted individually)
#   VIAL  – Vial         (injectable / parenteral)
#   BTL   – Bottle       (liquid, syrup, suspension, inhaler, drops, spray)
#   TUBE  – Tube         (topical cream / ointment / gel / paste)
#   SACH  – Sachet       (powder / granules dispensed in sachets)
#   MG    – Milligram    (not assigned from dosage_form; used for raw ingredients)
#   ML    – Milliliter   (not assigned from dosage_form; used for raw ingredients)
#   STRIP – Strip        (derived: 10 TAB; assigned manually if needed)
#   BOX   – Box          (derived: 10 STRIP; assigned manually if needed)

DOSAGE_FORM_TO_UNIT_CODE: list[tuple[str, str]] = [
    # ── Capsules (before tablet catch-all) ────────────────────────────────
    ("CAPSULE", "CAP"),
    ("CAPSULES", "CAP"),
    # ── Parenteral / injectable ───────────────────────────────────────────
    ("PRE-FILLED PEN", "VIAL"),  # insulin pens etc.
    ("PRE-FILLED SYRINGES", "VIAL"),
    ("PRE-FILLED SYRINGE", "VIAL"),
    ("DRY PRE-FILLED", "VIAL"),
    ("INFUSION VIALS", "VIAL"),
    ("INFUSION BAGS", "VIAL"),
    ("INFUSION AMPOULES", "VIAL"),
    ("INFUSION DRY VIALS", "VIAL"),
    ("INFUSION BOTTLES", "VIAL"),
    ("SOLUTION FOR INFUSION", "VIAL"),
    ("SUSPENSION FOR INTRATRACHEAL", "VIAL"),
    ("I M AMPOULES", "VIAL"),
    ("I V AMPOULES", "VIAL"),
    ("I V VIALS", "VIAL"),
    ("I M VIALS", "VIAL"),
    ("S C VIALS", "VIAL"),
    ("S C AMPOULES", "VIAL"),
    ("I M PRE-FILLED SYRINGES", "VIAL"),
    ("DRY AMPOULES", "VIAL"),
    ("COMBINATION PACK VIALS", "VIAL"),
    ("CARTRIDGES", "VIAL"),
    ("CARTRIDGE", "VIAL"),
    ("IMPLANTS", "VIAL"),
    ("IMPLANT", "VIAL"),
    ("PARENTERAL", "VIAL"),  # broad catch-all for PARENTERAL *
    ("AMPOULE", "VIAL"),
    ("VIAL", "VIAL"),
    ("INJECTION", "VIAL"),
    # ── Inhalers (dispensed as a bottle/device) ────────────────────────────
    ("METERED-DOSE INHALER", "BTL"),
    ("METERED-DOSE POWDER INHALER", "BTL"),
    ("METERED-DOSE LIQUIDS", "BTL"),
    ("LIQUID FOR INHALATION", "BTL"),
    ("INHALER CAPSULES", "BTL"),  # inhaler caps go in BTL (device)
    ("INHALER", "BTL"),
    ("INHALATION", "BTL"),
    # ── Topical solids → TUBE ─────────────────────────────────────────────
    ("EMULSION GEL", "TUBE"),
    ...
    ("BALM", "TUBE"),
    ("BALMS", "TUBE"),
    # ── Sachets / powders / granules → SACH ───────────────────────────────
    ("EFFERVESCENT POWDERS", "SACH"),
    ("UNIT DOSE POWDERS", "SACH"),
    ("SOLUBLE POWDERS", "SACH"),
    ("OTHER POWDERS", "SACH"),
    ("COMBINATION PACK POWDERS", "SACH"),
    ("GRANULES", "SACH"),
    ("GRANULE", "SACH"),
    ("POWDER", "SACH"),
    ("POWDERS", "SACH"),
    ...
    ("SACHETS", "SACH"),
    # ── Liquids / bottles ─────────────────────────────────────────────────
    ("DRY SUSPENSIONS", "BTL"),  # reconstituted suspensions
    ("SUSPENSIONS", "BTL"),
    ("SUSPENSION", "BTL"),
    ("SYRUPS", "BTL"),
    ("SYRUP", "BTL"),
    ...
    ("MOUTH WASH", "BTL"),
    ("MOUTHWASH", "BTL"),
    ("NASAL", "BTL"),  # nasal drops/sprays/liquids
    ("OPHTHALMIC", "BTL"),  # eye drops/suspensions/liquids
    ("OTIC", "BTL"),  # ear drops
    # ── Suppositories / pessaries → TAB (solid individual unit) ───────────
    ("SUPPOSITORIES", "TAB"),
    ("SUPPOSITORY", "TAB"),
    ("PESSARIES", "TAB"),
    ("PESSARY", "TAB"),
    # ── IUDs / devices → TAB (individual solid unit) ──────────────────────
    ("INTRA UTERINE DEVICES", "TAB"),
    ("INTRA UTERINE DEVICE", "TAB"),
    # ── Tablets (all variants) ─────────────────────────────────────────────
    ("LOZENGES", "TAB"),
    ("LOZENGE", "TAB"),
    ("PELLETS", "TAB"),
    ("PELLET", "TAB"),
    ...
    ("TABLET", "TAB"),
    ("TABLETS (COATED", "TAB"),
    # ── Special / unknown → TAB (safest pharma default) ───────────────────
    ("SPECIAL SOLIDS", "TAB"),
]


def dosage_form_to_unit_code(dosage_form: str | None) -> str:
    """
    Return the best unit short_code for a given NDA dosage_form string.
    Case-insensitive substring matching; first match wins.
    Falls back to 'TAB' (most common pharma unit).
    """
    if not dosage_form:
        return "TAB"
    df_upper = dosage_form.upper()
    for keyword, code in DOSAGE_FORM_TO_UNIT_CODE:
        if keyword.upper() in df_upper:
            return code
    return "TAB"
...
_ROLES = [
    (
        2,
        "Company Admin",
        "Full access to all company data and all pages",
        90,
        "company",
    ),
    (
...
# ── 6. Units ──────────────────────────────────────────────────────────────────
# These are the TEMPLATE units (company_id = 1).
# When a new company registers, copy_all_template_data_to_new_company() clones
# these rows with the new company_id — so every company has its own unit set.
#
# The scraper never writes unit_id (it doesn't know company IDs).
# Instead it writes unit_code (e.g. "TAB") which views resolve at query time
# via:  JOIN units ON short_code = unit_code AND company_id = <querying company>
...
_INCOME_CATS = [
    ...
    (9, "Consultation Fees"),
    (10, "Vaccine & Immunization Services"),
    (11, "Online/Telepharmacy Sales"),
    (12, "Insurance Reimbursements"),
    (13, "Government Subsidies/Grants"),
    (14, "Other Income"),
    (15, "Prescription Drugs"),
    (16, "Over-the-Counter (OTC)"),
]
...
_EXPENSE_CATS = [
    ...
    (11, "Office Supplies"),
    (12, "Transportation & Delivery"),
    (13, "Taxes & Duties"),
    (14, "Training & Staff Development"),
    (15, "Depreciation"),
    (16, "Inventory Loss/Shrinkage"),
    (17, "Waste Disposal"),
    (18, "Software & IT Subscriptions"),
    (19, "Patient Assistance Programs"),
    (20, "Other Expenses"),
    (21, "Medicine Purchases"),
    (22, "Supplies Purchases"),
    (23, "Pharmacist Salaries"),
    (24, "Support Staff Wages"),
]
...
# ── FK-safe pipeline ──────────────────────────────────────────────────────────
SEED_PIPELINE: list[tuple[str, Callable[[], None]]] = [
