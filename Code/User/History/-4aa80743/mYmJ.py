...
def seed_role_permissions(company_id: int) -> None:
    ...
    # Get this company's role IDs by name
    company_roles = (
        client.table("roles")
        .select("id,role_name")
        .eq("company_id", company_id)
...
def seed_units(company_id: int) -> None:
    ...
    # Insert/get base units and track their IDs
    ...
    # Derived units referencing base unit IDs from this company
    tab_id = unit_id_map.get("TAB")
    strip_id = None  # we'll get this after inserting STRIP
    ml_id = unit_id_map.get("ML")
...
# ─────────────────────────────────────────────────────────────────────────────
# 10. Products  ← NEW — seeded directly, no NDA dependency
...
def seed_products(company_id: int) -> None:
    ...
    # Resolve category and unit IDs for this company
    cats = (
        client.table("categories")
    ...
    for i, (
        ...
        # is_restricted alternates: odd index = True, even = False  (for testing)
        is_restricted = i % 2 == 0
...
def seed_inventory_batches(company_id: int) -> None:
    ...
    for prod in products:
        for j, store_id in enumerate(store_ids):
            ...
            # Expiry: some near-expiry, some good, some expired
            if j == 0:
            ...
            row = {
                ...
                # batch inherits product's is_restricted so views filter correctly
...
# ─────────────────────────────────────────────────────────────────────────────
# 12. Sales  ← NEW
#     Mix of is_restricted=True and False, different stores, customers, statuses
# ─────────────────────────────────────────────────────────────────────────────
def seed_sales(company_id: int) -> None:
    ...
    sale_templates = [
        ...
        (3, "credit", "partial", "completed", False),
    ...
    for idx, (
        ...
        row = {
            ...
            "amount_paid": total
            if pay_status == "paid"
            else (total // 2 if pay_status == "partial" else 0),
...
# ─────────────────────────────────────────────────────────────────────────────
# 13. Purchase orders + returns  ← NEW
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 15. Expenses  ← NEW with is_restricted
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 16. Income  ← NEW with is_restricted
# ─────────────────────────────────────────────────────────────────────────────
...
def make_pipeline(company_id: int) -> list[tuple[str, Callable[[], None]]]:
    return [
        ...
        ("permissions", seed_permissions),  # global
...
# (points to template company CID)
