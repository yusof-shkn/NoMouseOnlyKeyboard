...
export const LoginCard = styled.div<{ $lifted?: boolean }>`
  ...
  @media (max-width: 800px) {
    ...
    /* Default: bottom-sheet — tall enough to not look like a tiny chip */
    border-radius: 28px 28px 0 0;
    padding: 2rem 1.5rem 3.5rem;
    min-height: 60vh;

    /* Lifted: fixed full-screen white card.
     * CRITICAL: Do NOT set overflow-y:auto here. iOS Safari creates a native
     * UIScrollView for position:fixed + overflow:auto, which swallows touch
     * events and prevents the native <select> picker from opening.
     * Scrolling is handled by CardScrollInner below instead. */
    ${(p) =>
      p.$lifted
        ? `
      position: fixed;
...
// ─── Inner scroll wrapper for lifted card ─────────────────────────────────
// Lives *inside* LoginCard. Only this element gets overflow-y:auto so the
// position:fixed card itself stays overflow:visible — that's what lets the
// native <select> picker open on iOS Safari / Android Chrome.
export const CardScrollInner = styled.div<{ $lifted?: boolean }>`
  @media (max-width: 800px) {
    ${(p) =>
      p.$lifted
        ? `
...
export default function PortalLoginLayout({
  ...
  const handleFocusIn = useCallback((e: React.FocusEvent) => {
    ...
    // Never lift on button or select focus — country trigger is a button,
    // lifting while the trigger is clicked causes layout shift that breaks
    // the dropdown position calculation
  ...
  const handleFocusOut = useCallback((e: React.FocusEvent) => {
    ...
    if (tag === 'button' || tag === 'select') return
    if (tag === 'input' || tag === 'textarea') {
      // If focus is moving to a button (country trigger), don't un-lift —
      // the user is opening the country picker
      const next = e.relatedTarget as HTMLElement | null
      if (next && next.tagName.toLowerCase() === 'button') return
      blurTimerRef.current = setTimeout(() => setLifted(false), 400)
    }
  }, [])
  ...
  return (
    <Page>
      ...
      <FormPanel $grad={gradient}>
        ...
        >
          <CardScrollInner $lifted={lifted}>{children}</CardScrollInner>
        </LoginCard>
      </FormPanel>
    </Page>
  )
