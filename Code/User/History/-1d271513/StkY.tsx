...
const MobileHero = styled.div<{ $lifted: boolean }>`
  ...
  @media(max-width:800px){
    ...
    position: relative;
    flex-shrink: 0;
    padding: 0.25rem 1.5rem 3.25rem;
...
const MobileOrb = styled.div<{ $size:number; $top:string; $left:string }>`
  ...
  background:rgba(255,255,255,0.06);top:${p=>p.$top};left:${p=>p.$left};pointer-events:none;
`
const PillIcon = styled.div`
  width:64px;height:64px;border-radius:20px;
  background:rgba(255,255,255,0.18);border:1.5px solid rgba(255,255,255,0.3);
  display:flex;align-items:center;justify-content:center;
  margin-bottom:18px;animation:${float} 4s ease-in-out infinite;
...
export const LoginCard = styled.div<{ $lifted?: boolean }>`
  ...
  border:1px solid #EDF0F7;animation:${fadeUp} 0.5s ease both;
...
export const LoginInputWrap = styled.div`
  .mantine-TextInput-label,.mantine-PasswordInput-label,.mantine-Select-label {
  ...
  .mantine-PasswordInput-innerInput {
    height:48px;border:1.5px solid #E4E9F0;border-radius:12px;
...
export const LoginPrimaryBtn = styled.button`
  ...
  &:hover:not(:disabled){opacity:0.92;transform:translateY(-1px);box-shadow:0 6px 24px rgba(21,179,224,0.45);}
...
interface PortalLink { to: string; label: string }
    ...
    <Page>
      ...
      <BrandPanel $grad={gradient}>
        ...
        <Box style={{ display:'flex', alignItems:'center', gap:10, marginBottom:52 }}>
          ...
          <Text style={{ fontFamily:"'DM Sans',sans-serif", fontWeight:800, fontSize:20, color:'#fff', letterSpacing:'-0.3px' }}>
            MedDeliverySOS
          </Text>
        </Box>

        <Box style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(255,255,255,0.12)', border:'1px solid rgba(255,255,255,0.22)', borderRadius:20, padding:'5px 14px', marginBottom:20, width:'fit-content' }}>
          {portalIcon}
          <Text style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, fontWeight:700, color:'rgba(255,255,255,0.9)', textTransform:'uppercase', letterSpacing:'0.08em' }}>
            {portalLabel}
          </Text>
        </Box>

        <Text style={{ fontFamily:"'DM Sans',sans-serif", fontWeight:800, fontSize:30, color:'#fff', lineHeight:1.2, marginBottom:12 }}>
          {headline}
        </Text>
        <Text style={{ fontFamily:"'DM Sans',sans-serif", fontSize:14, color:'rgba(255,255,255,0.65)', marginBottom:44, lineHeight:1.75 }}>
          {subheadline}
        </Text>
        ...
        <Text style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, color:'rgba(255,255,255,0.45)', textTransform:'uppercase', letterSpacing:'0.08em', marginBottom:10 }}>
          Other portals
        </Text>
        <Stack gap={8}>
          {otherPortals.map(({ to, label }) => (
            <PortalChip key={to} to={to}>
              <Text style={{ fontFamily:"'DM Sans',sans-serif", fontSize:13, fontWeight:600, color:'#fff' }}>{label}</Text>
              <TbArrowRight size={14} color="rgba(255,255,255,0.6)"/>
            </PortalChip>
          ))}
        </Stack>
      </BrandPanel>
      ...
      <FormPanel $grad={gradient}>
        ...
        <MobileHero $lifted={lifted}>
          <MobileOrb $size={300} $top="-80px" $left="-80px" />
          <MobileOrb $size={200} $top="30%" $left="70%" />
          <MobileOrb $size={150} $top="70%" $left="-20px" />
          <PillIcon>{portalIcon}</PillIcon>
          <Text style={{ fontFamily:"'DM Sans',sans-serif", fontWeight:800, fontSize:lifted ? 16 : 22, color:'#fff', lineHeight:1.2, marginBottom:4, transition:'font-size 0.3s ease' }}>
            {portalLabel}
          </Text>
          {!lifted && (
            <Text style={{ fontFamily:"'DM Sans',sans-serif", fontSize:13, color:'rgba(255,255,255,0.7)', lineHeight:1.6 }}>
              MedDeliverySOS
            </Text>
          )}
        </MobileHero>
