...
/**
 * Enrich raw cash_flow_transactions rows with derived and joined fields.
 *
 * Derives: cash_in, cash_out, net_cash_flow, running_balance
 * Joins:   store_name (via store_id), creator_name (via created_by)
 *
 * NOTE: account_id does not exist on cash_flow_transactions — account_name
 * cannot be looked up from these rows. It is set to 'N/A' unless a future
 * column is added.
 */
export const enrichCashFlowTransactions = (
  ...
  profiles: Profile[],
  stores: Store[],
  accounts: ChartOfAccount[], // kept for API compat; not usable without account_id
): CashFlowTransaction[] => {
  ...
  return transactions.map((tx): CashFlowTransaction => {
    ...
    // created_by is bigint (user id), not auth_id — match accordingly
    ...
    return {
      ...
      account_name: tx.account_name ?? 'N/A', // no account_id on table
      account_code: tx.account_code ?? 'N/A',
...
/**
 * Fetch and enrich cash flow data.
 */
...
export const formatDate = (
  ...
  return new Date(dateString).toLocaleDateString(getCurrencyLocale(currency), {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
  })
...
export const formatDateTime = (
  ...
  return new Date(dateString).toLocaleString(getCurrencyLocale(currency), {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
