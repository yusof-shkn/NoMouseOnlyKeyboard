...
export async function fetchProductsWithInventoryPaginated(
  ...
  // Fetch available stock per product (non-expired batches only)
  ...
  const productsWithInventory = products.map((product) => {
    ...
    } else if (inv.total_quantity <= product.reorder_level) {
...
export async function createSaleWithItems(request: {
  ...
  // IMPORTANT: The database function parameter should be VARCHAR
  // and the function itself should handle the casting to sale_type enum
  // If you're getting enum errors, check your database function signature
