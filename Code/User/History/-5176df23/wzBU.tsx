...
const RolesPermissions = () => {
  ...
  const [permissionCoverageFilter, setPermissionCoverageFilter] =
    useState('all')
  ...
  const fetchRoles = useCallback(async () => {
    ...
    try {
      ...
      const rolesResult = await fetchRolesData({
        ...
        permissionCoverage: permissionCoverageFilter,
      })
  ...
  const handleLocalDelete = useCallback(
    async (row: EnrichedRole): Promise<void> => {
  ...
  const filters = useMemo(
    () =>
      getRoleFilters({
        ...
        onPermissionCoverageChange: (value) => {
          setPermissionCoverageFilter(value)
          setCurrentPage(1)
        },
