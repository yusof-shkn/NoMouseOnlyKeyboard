...
import {
  Box, Text, Stack, Loader, Center, CopyButton, Tooltip,
  ActionIcon, Badge, Group, Divider,
} from '@mantine/core'
import { TbCheck, TbCopy, TbExternalLink, TbPhone, TbLock, TbUserCheck } from 'react-icons/tb'
...
export default function InsuranceOrderOnBehalf(): JSX.Element {
  ...
  return (
    <Box p={{ base: 'md', sm: 'xl' } } style={{ maxWidth: 700, margin: '0 auto' }}>
      <Text
        style={{
      ...
      <Text c="dimmed" size="sm" mb="xl">
        For patients who do not have a smartphone — use the shared customer account below.
      </Text>
      ...
      {account ? (
        <Stack gap="xl">
          ...
          <Card>
            <Group gap={10} mb="md">
              <TbUserCheck size={20} color={SavedColors.Primaryblue} />
              <Text style={{ fontFamily: "'Nunito',sans-serif", fontWeight: 800, fontSize: 15, color: SavedColors.TextColor }}>
            ...
            <Stack gap="sm">
              {/* Phone */}
              <CredRow>
                <TbPhone size={18} color={SavedColors.Primaryblue} style={{ flexShrink: 0 }} />
                <Box style={{ flex: 1 }}>
                  <Label>Phone Number</Label>
                  <Value>{account.phone}</Value>
                </Box>
                <CopyButton value={account.phone} timeout={2000}>
                  {({ copied, copy }) => (
              ...
              {/* Password */}
              <CredRow>
                <TbLock size={18} color={SavedColors.Primaryblue} style={{ flexShrink: 0 }} />
                <Box style={{ flex: 1 }}>
                  <Label>Password</Label>
                  <Value>Customer1234!</Value>
                </Box>
                <CopyButton value="Customer1234!" timeout={2000}>
                  {({ copied, copy }) => (
                    <Tooltip label={copied ? 'Copied!' : 'Copy'} withArrow>
                      <ActionIcon
                        color={copied ? 'teal' : 'gray'}
            ...
            <Stack gap="sm">
              {steps.map((step, i) => (
                <Group key={i} gap={12} align="flex-start">
                  ...
                  <Text size="sm" c="dimmed" style={{ flex: 1, paddingTop: 3 }}>{step}</Text>
                </Group>
              ))}
            </Stack>
          </Box>
        ...
        <Card>
          <Text c="dimmed" size="sm">
            No service account found for your insurance provider. Please contact your administrator.
          </Text>
        </Card>
      )}
    </Box>
