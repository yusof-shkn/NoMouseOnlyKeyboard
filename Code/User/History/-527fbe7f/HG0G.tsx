...
import {
  Box, Button, Container, Group, Stack, Text, Textarea, TextInput,
  Progress, Image, ActionIcon, Radio, Badge, Alert, Divider,
} from '@mantine/core'
import {
  TbArrowLeft, TbUpload, TbPhoto, TbFileDescription, TbX,
  TbMapPin, TbPlus, TbAlertCircle, TbCheck, TbClock,
  TbShieldOff, TbCurrencyDollar, TbPhone, TbPencil, TbCamera,
} from 'react-icons/tb'
...
export default function UploadPrescription() {
  ...
  useEffect(() => {
    if (profiles.length > 0) {
      ...
      reset((prev: FormValues) => ({ ...prev, profileId: prev.profileId || preferred }))
    }
  ...
  const handleNewAddress = async (addr: AddressData) => {
    ...
    const existing = historyAddrs.find(
      h => h.street.toLowerCase() === addr.street.toLowerCase() && h.city?.toLowerCase() === addr.city?.toLowerCase()
    )
    let savedId: string
    if (existing) {
  ...
  const handleFile = async (f: File) => {
    if (!ACCEPTED.includes(f.type)) {
      notifications.show({ title: 'Invalid file type', message: 'Please upload a JPG, PNG, WebP or GIF image.', color: 'orange' })
      return
    }
    if (f.size > MAX_SIZE) {
      notifications.show({ title: 'File too large', message: 'Max file size is 10 MB.', color: 'orange' })
      return
    }
  ...
  const onSubmit = async (values: FormValues) => {
    if (!user) return
    if (!file) {
      notifications.show({ title: 'No image selected', message: 'Please upload a prescription image.', color: 'orange' })
      return
    }
    ...
    return (
      ...
      >
        <Box style={{
          ...
          {selected ? <TbCheck size={16} color="#fff" /> : <TbMapPin size={16} color="#9ca3af" />}
        </Box>
        <Box style={{ flex: 1, minWidth: 0 }}>
          <Group gap={6} mb={2}>
            ...
            {addr.source === 'history' && (
              <Badge size="xs" color="gray" variant="light" leftSection={<TbClock size={9}/>}>Recent</Badge>
            )}
          </Group>
          <Text size="xs" c="dimmed">
            {addr.street}{addr.city ? `, ${addr.city}` : ''}{addr.zone ? ` — ${addr.zone}` : ''}
          </Text>
          {addr.instructions && (
            <Text size="xs" c="dimmed" mt={2} style={{ fontStyle: 'italic' }}>{addr.instructions}</Text>
          )}
        </Box>
        <Radio
          checked={selected} onChange={() => setValue('selectedAddr', addr.id)}
          color="lotusCyan" style={{ pointerEvents: 'none' }}
        />
      </Box>
    )
  }
  ...
  const savedPercent      = originalSize > 0 && resizedSize < originalSize
    ...
    <PageWrapper>
      <PageHeader style={{ top: 60 }}>
        <Container size="md">
          <Button variant="subtle" leftSection={<TbArrowLeft size={16}/>}
            onClick={() => navigate('/dashboard')} style={{ color: '#fff' }}>Back</Button>
        </Container>
      </PageHeader>
      ...
      <Container size="md" py="xl">
        <Box mb="xl">
          ...
          <BlueDivider />
          <Text c="dimmed" size="sm" mt="xs">Upload a clear photo of your prescription for pharmacy review</Text>
        </Box>
        ...
        <ContentCard>
          <form onSubmit={handleSubmit(onSubmit)}>
            <Stack gap="lg">
              ...
              {/* ── Profile ── */}
              <FormSelect
                name="profileId" control={control}
                label="For Profile" labelColor={SavedColors.TextColor}
                data={profiles.map(p => ({ value: p.id, label: p.name + (p.is_main_account ? ' (Me)' : '') }))}
              />

              {/* ── Prescription image ── */}
              <Box>
                <Text size="sm" fw={600} mb="xs" c={SavedColors.TextColor}>
                  Prescription Image <Text component="span" c="red">*</Text>
                </Text>
                ...
                {!preview ? (
                  <>
                    ...
                    <Box
                      ...
                      style={{
                        border: `2px dashed ${SavedColors.Primaryblue}`, borderRadius: 10,
                        padding: '32px 24px', textAlign: 'center', cursor: 'pointer',
                        background: SavedColors.lightBlue,
                      }}
                    >
                      <TbUpload size={36} color={SavedColors.Primaryblue} style={{ margin: '0 auto 10px', display: 'block' }}/>
                      <Text size="sm" fw={600} c={SavedColors.Primaryblue}>Click to upload or drag & drop</Text>
                      <Text size="xs" c="dimmed" mt={4}>JPG, PNG, WebP, GIF — max 10 MB</Text>
                    </Box>
                    ...
                    >
                      <Box style={{
                        ...
                        <TbCamera size={20} color="#fff" />
                      </Box>
                      <Box style={{ flex: 1 }}>
                        <Text size="sm" fw={700} c={SavedColors.TextColor}>Take a Photo</Text>
                        <Text size="xs" c="dimmed">Open camera to photograph your prescription</Text>
                      </Box>
                      <Badge size="xs" color="lotusCyan" variant="light">Camera</Badge>
                    </Box>
                  </>
                ) : (
                  /* Preview */
                  <Box style={{ position: 'relative' }}>
                    {resizing ? (
                      <Box style={{
                        height: 200, borderRadius: 8, background: '#f0f4f8',
                        ...
                        <Text size="xs" c="dimmed">Optimising image…</Text>
                      </Box>
                    ) : (
                      <Image src={preview} alt="Preview" radius="md"
                        style={{ maxHeight: 320, objectFit: 'contain', background: '#f0f4f8', border: `1px solid ${SavedColors.DemWhite}` }}/>
                    )}

                    <ActionIcon color="red" variant="filled" radius="xl" size="sm"
                      style={{ position: 'absolute', top: 8, right: 8 }} onClick={clearFile}>
                      <TbX size={12}/>
                    </ActionIcon>

                    {!resizing && (
                      <Group mt="xs" gap="xs" wrap="wrap">
                        <TbPhoto size={14} color={SavedColors.Primaryblue}/>
                        <Text size="xs" c="dimmed">
                          {file?.name} — {(resizedSize / 1024).toFixed(0)} KB
                          {savedPercent > 0 && (
                            <Text component="span" c="teal" fw={600}> (optimised, {savedPercent}% smaller)</Text>
                          )}
                        </Text>
                        <Button size="xs" variant="subtle" color="lotusCyan"
                          onClick={() => fileInputRef.current?.click()}>Change</Button>
                        <Button size="xs" variant="subtle" color="lotusCyan"
                          leftSection={<TbCamera size={12}/>}
                          onClick={() => cameraInputRef.current?.click()}>Retake</Button>
                      </Group>
                    )}
                  </Box>
                )}
              ...
              <Box>
                ...
                {loadingAddrs ? (
                  <Group gap="xs" py="sm">
                    <div style={{ width: 16, height: 16, borderRadius: '50%', border: '2px solid #15B3E0', borderTopColor: 'transparent', animation: 'spin 0.7s linear infinite' }}/>
                    <Text size="xs" c="dimmed">Loading addresses…</Text>
                  </Group>
                ) : allAddresses.length === 0 ? (
                  <Stack gap="xs">
                    <Text size="sm" c="dimmed">No saved addresses yet.</Text>
                    <Group gap="xs">
                      <Button size="xs" color="lotusCyan" variant="light" leftSection={<TbMapPin size={12}/>}
                        onClick={() => setPickerOpen(true)}>
                        Enter Address
                      </Button>
                      <Button size="xs" color="blue" variant="subtle"
                  ...
                  <Stack gap="xs">
                    {profileAddrs.length > 0 && (
                      <>
                        ...
                        {profileAddrs.map(a => renderAddrCard({
                          source: 'profile', id: a.id, label: a.label,
                          street: a.street, city: a.city, zone: a.zone,
                          instructions: a.instructions ?? null,
                    ...
                    {historyAddrs.length > 0 && (
                      <>
                        ...
                        {historyAddrs.map(a => renderAddrCard({
                          source: 'history', id: a.id, label: a.label,
                          street: a.street, city: a.city, zone: a.zone,
                          instructions: a.instructions,
                ...
                {errors.selectedAddr && (
                  <Text size="xs" c="red" mt={4}>{errors.selectedAddr.message}</Text>
                )}
              </Box>
              ...
              <Box>
                ...
                <Stack gap="xs">
                  ...
                  {profilePhone && (
                    ...
                    >
                      <Box style={{
                        width: 36, height: 36, borderRadius: '50%', flexShrink: 0,
                        background: !editingPhone ? SavedColors.Primaryblue : '#f3f4f6',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                      }}>
                        {!editingPhone
                          ? <TbCheck size={16} color="#fff" />
                          : <TbPhone size={16} color="#9ca3af" />
                        }
                      </Box>
                      <Box style={{ flex: 1, minWidth: 0 }}>
                        <Group gap={6} mb={2}>
                          <Text size="sm" fw={700} c={SavedColors.TextColor}>{profilePhone}</Text>
                          <Badge size="xs" color="lotusCyan" variant="light">Profile</Badge>
                        </Group>
                        <Text size="xs" c="dimmed">Saved contact for {activeProfile?.name ?? 'this profile'}</Text>
                      </Box>
                      <Radio
                        checked={!editingPhone}
                        onChange={() => { setValue('contactPhone', profilePhone); setEditingPhone(false) }}
                        color="lotusCyan"
                  ...
                  >
                    ...
                    <Box style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px' }}>
                      <Box style={{
                        width: 36, height: 36, borderRadius: '50%', flexShrink: 0,
                        background: editingPhone ? SavedColors.Primaryblue : '#f3f4f6',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        ...
                        {editingPhone
                          ? <TbCheck size={16} color="#fff" />
                          : <TbPencil size={16} color="#9ca3af" />
                        }
                      </Box>
                      <Box style={{ flex: 1 }}>
                        <Text size="sm" fw={700} c={SavedColors.TextColor}>Use a different number</Text>
                        <Text size="xs" c="dimmed">Enter any contact number for this order</Text>
                      </Box>
                      <Radio
                        checked={editingPhone}
                    ...
                    {editingPhone && (
                      <Box
                        style={{ borderTop: `1px solid ${SavedColors.DemWhite}`, padding: '10px 14px 14px' }}
                      ...
                      >
                        <Controller name="contactPhone" control={control} render={({ field }) => (
                          <TextInput
                            ...
                            leftSection={<TbPhone size={14} color={SavedColors.Primaryblue} />}
                            autoFocus
                          />
                        )} />
                ...
                {!profilePhone && errors.contactPhone && (
                  <Text size="xs" c="red" mt={4}>{errors.contactPhone.message}</Text>
                )}
              </Box>

              {/* ── Payment method ── */}
              <Box>
                <Text size="sm" fw={600} mb="xs" c={SavedColors.TextColor}>Payment Method</Text>
                <Stack gap="xs">
                  <Box
                  ...
                  >
                    <Radio checked={!cashOnly} onChange={() => setCashOnly(false)} color="lotusCyan" style={{ pointerEvents: 'none' }} />
                    <Box style={{ flex: 1 }}>
                      <Text size="sm" fw={700} c={SavedColors.TextColor}>Use Insurance (if available)</Text>
                      <Text size="xs" c="dimmed">The pharmacy will apply your insurance coverage to reduce costs</Text>
                    </Box>
                  </Box>
                  ...
                  >
                    <Radio checked={cashOnly} onChange={() => setCashOnly(true)} color="orange" style={{ pointerEvents: 'none' }} />
                    <Box style={{ flex: 1 }}>
                      <Group gap={6}>
                        <TbCurrencyDollar size={15} color="#D97706" />
                        <Text size="sm" fw={700} c="#92400E">Pay Cash (No Insurance)</Text>
                      </Group>
                      <Text size="xs" c="dimmed">Skip insurance — pay full cash price at confirmation</Text>
                    </Box>
                    {cashOnly && (
                      <Badge color="orange" variant="light" size="xs" leftSection={<TbShieldOff size={9} />}>
                        Cash Only
                      </Badge>
                    )}
                  </Box>
              ...
              {/* ── Notes ── */}
              <Box>
                <Text size="sm" fw={600} mb="xs" c={SavedColors.TextColor}>Notes for Pharmacist</Text>
                <Controller name="notes" control={control} render={({ field }) => (
                  <Textarea {...field} placeholder="Any allergies, preferred brands, or notes…" rows={3} radius="sm"/>
                )}/>
              </Box>

              {uploading && progress < 100 && (
                <Progress value={progress} color="lotusCyan" size="sm" animated/>
              )}

              <Group justify="flex-end" gap="sm">
                <Button variant="default" onClick={() => navigate('/dashboard')} radius={8}>Cancel</Button>
                <Button type="submit" loading={isSubmitting || uploading || resizing} radius={8}
