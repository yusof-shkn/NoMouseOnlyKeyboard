...
/**
 * ✅ CLEANED: Mark purchase order as paid (no partial_paid logic)
 */
export const markPurchaseOrderAsPaid = async (
  ...
  try {
    ...
    // ✅ CLEANED: Simple binary payment status (paid or unpaid)
    ...
    for (const item of poItems) {
      ...
      if (currentBatch) {
        const newQuantityReceived =
          currentBatch.quantity_received + item.quantity_ordered
        const newQuantityAvailable =
          currentBatch.quantity_available + item.quantity_ordered
