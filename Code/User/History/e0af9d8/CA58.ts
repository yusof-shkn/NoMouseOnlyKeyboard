...
export const markPurchaseOrderAsPaid = async (
  ...
  try {
    ...
    const paymentStatus =
      newPaidAmount >= currentPO.total_amount ? 'paid' : 'unpaid'
    ...
    // ─── Step 1: Insert into payment_transactions ─────────────────────────
    // The trigger fn_create_payment_journal_entry fires automatically on INSERT:
    //   DR Accounts Payable 2100
    //   CR Cash/Bank (based on payment_method)
    // This updates v_trial_balance and v_balance_sheet automatically.
    ...
    // ─── Step 2: Insert into cash_flow_transactions ───────────────────────
    // v_cash_flow reads from cash_flow_transactions (not payment_transactions).
    // This insert also removes the PO from the view's purchase_orders branch
    // (which excludes POs with an existing cash_flow_transactions row),
    // preventing double-counting in cash flow reports.
    const { error: cftError } = await supabase
      ...
      .insert({
        ...
        transaction_type: 'purchase', // view maps anything not in [inflow/sale/receipt/income] → outflow
        ...
        journal_entry_id: paymentData.journal_entry_id, // use the one the trigger just set
    ...
    if (cftError) {
      // Rollback payment_transactions — journal entry will cascade or be orphaned
      // but better than silent failure on cash flow
      await supabase
        .from('payment_transactions')
        .delete()
    ...
    console.log('✅ PO payment complete:', {
      ...
      newPaidAmount,
      paymentStatus,
      journalEntryId: paymentData.journal_entry_id, // set by trigger
      paymentMethod,
    })
