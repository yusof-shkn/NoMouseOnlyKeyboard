...
export const createPurchaseReturn = async (
  ...
  try {
    ...
    // Always mark PO as 'returned' once a return is created (regardless of approval flow)
    if (returnData.purchase_order_id) {
      await supabase
        .from('purchase_orders')
        .update({ status: 'returned', updated_at: new Date().toISOString() })
        .eq('id', returnData.purchase_order_id)
      console.log('✅ PO status set to returned')
    }

    if (!requiresApproval) {
      console.log(
        '🔄 Auto-approved: Processing inventory and balance adjustments...',
      )

      if (companySettings.auto_restock_on_return) {
        console.log('📦 Auto-restocking items...')
        await restockReturnedItems(returnRecord.id, returnData.items)
        console.log('✅ Items restocked')
      }

      console.log('💰 Adjusting supplier balance...')
      await adjustSupplierBalanceForReturn(
        returnData.supplier_id,
        totalRefund,
        returnRecord.id,
        'approve',
      )
      console.log('✅ Supplier balance adjusted')
    } else {
      console.log(
...
/**
 * Approve purchase return using database function
 */
export const approvePurchaseReturn = async (
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('approve_purchase_return', {
      p_return_id: id,
    ...
    // Get the purchase_order_id from the return and update PO status to 'returned'
