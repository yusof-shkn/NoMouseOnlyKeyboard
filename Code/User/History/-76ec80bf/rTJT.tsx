...
import { SalesOrdersDrawer } from './components/SalesOrdersDrawer'
...
export const SalePOS: React.FC = () => {
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden',border:'1rem solid red' }} >
      <MainContent>
            {/* Products Section */}
            <ProductsSection
              style={{
      ...
      >
        <Tooltip
          label="View my sales orders for this store"
          position="right"
          withArrow
        >
          <Button
      ...
      <SalesOrdersDrawer
        ...
        onResumeDraft={(sale) => {
          // Load the draft's items back into the current POS session
          onResetOrder()
          const items = (sale.sale_items || []).map((item: any) => {
            // batches is aliased from product_batches in the query
            const batchData = item.batches || {}
            const productData = item.products || {}
            const unitData = productData.units || {}
            return {
              id: item.product_id,
              productName: productData.product_name || '',
              productCode: productData.product_code || '',
              ...
              expiryDate: batchData.expiry_date || null,
              availableStock: batchData.available_quantity || item.quantity,
              unit: unitData.short_code || item.unit_short_code || '',
            }
          })
          setOrderItems(items)
          if (sale.sale_type) setSaleType(sale.sale_type)
          if (sale.payment_method) setSelectedPayment(sale.payment_method as any)
          if (sale.customer_id) setSelectedCustomer(String(sale.customer_id))
          if (sale.discount_amount) setDiscount(sale.discount_amount)
          if (sale.sale_date) setSaleDate(sale.sale_date.slice(0, 10))
          notifications.show({
            title: 'Draft Resumed',
            message: `${sale.sale_number} loaded — complete when ready`,
            color: 'yellow',
