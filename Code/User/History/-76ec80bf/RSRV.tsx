...
export const SalePOS: React.FC = () => {
  ...
  // ── Load credit info when customer selected ──────────────────────────────
  useEffect(() => {
    if (selectedCustomer) {
      ...
      if (summary) {
        const utilization =
          summary.credit_limit > 0
            ? (summary.current_credit_balance / summary.credit_limit) * 100
            : 0

        if (summary.available_credit <= 0) {
          notifications.show({
            title: 'No Available Credit',
            message: `${summary.first_name} ${summary.last_name} has no available credit`,
            color: 'red',
            icon: <IconX size={18} />,
          })
          ...
          notifications.show({
            ...
            message: `${summary.first_name} ${summary.last_name} has used ${utilization.toFixed(0)}% of their credit limit`,
            color: 'yellow',
  ...
  // ── Credit validation before checkout ───────────────────────────────────
  ...
  const handleCreditPayment = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const paymentAmount = Number(formData.get('payment_amount'))
    const method = formData.get('payment_method') as string
    const reference = formData.get('payment_reference') as string
    ...
    setIsLoadingCredit(true)
    try {
      const currentBalance = selectedCustomerCredit.current_credit_balance
      const creditLimit = selectedCustomerCredit.credit_limit
      ...
      await supabase.from('credit_transactions').insert({
        ...
        status: 'completed',
        description: notes || `Credit payment via POS`,
        notes: notes || null,
      ...
      notifications.show({
        ...
        message: `Payment of ${defaultCurrency} ${paymentAmount.toLocaleString()} recorded successfully`,
        color: 'green',
      ...
      // Refresh credit info
      await loadCustomerCreditInfo(Number(selectedCustomer))
    } catch (error: any) {
  ...
  const handlePaymentSelect = async (value: string) => {
    ...
    // Warn if changing payment method in edit mode
    if (
      isEditMode &&
      originalPaymentMethod &&
      ...
      const confirmMessage = isCreditChange
        ? originalPaymentMethod === 'credit'
          ? `⚠️ Changing from CREDIT to ${value.toUpperCase()} will reverse the credit transaction and restore the customer's credit. Continue?`
          : `⚠️ Changing to CREDIT will deduct ${defaultCurrency} ${totals.grandTotal.toLocaleString()} from the customer's available credit. Continue?`
        : `⚠️ Changing payment method from "${originalPaymentMethod}" to "${value}". This may affect financial reports. Continue?`

      if (!confirm(confirmMessage)) return

      notifications.show({
        title: 'Payment Method Changed',
        message: `Changed from ${originalPaymentMethod} to ${value}`,
        color: 'orange',
        icon: <IconAlertTriangle size={18} />,
      })
    }
  ...
  const handleSaleTypeChange = (type: string) => {
    if (isEditMode && originalSaleType && type !== originalSaleType) {
      if (
        !confirm(
          `⚠️ Changing sale type from ${originalSaleType.toUpperCase()} to ${type.toUpperCase()}. This may affect payment method and customer association. Continue?`,
        )
      )
        return
      ...
      if (originalSaleType === 'wholesale' && type === 'retail') {
        ...
        if (selectedPayment === 'credit') {
          ...
          notifications.show({
            title: 'Payment Method Changed',
            message: 'Credit payment changed to cash for retail sale',
            color: 'blue',
      ...
      notifications.show({
        ...
        message: `Changed from ${originalSaleType} to ${type}`,
        color: 'orange',
        icon: <IconAlertTriangle size={18} />,
      })
    }
  ...
  return (
    <AppContainer>
              ...
              >
                ...
                >
                  <Stack gap="sm">
                    ...
                    {saleType === 'wholesale' && (
                      <>
                        ...
                        >
                          ...
                          {/* Show credit payment button if customer has outstanding balance */}
                          {selectedCustomerCredit &&
                            selectedCustomerCredit.current_credit_balance >
                              0 && (
                              <ActionIcon
                                size="lg"
                                variant="light"
                        ...
                        {selectedCustomerCredit &&
                          selectedCustomerCredit.credit_limit > 0 && (
                            <Paper
                              ...
                              style={{
                                backgroundColor:
                                  selectedCustomerCredit.available_credit <= 0
                                    ? isDark
                                      ? theme.colors.red[9]
                                      : theme.colors.red[0]
                                    : isDark
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  {defaultCurrency}{' '}
                                  {selectedCustomerCredit.current_credit_balance.toLocaleString()}
                                </Text>
                              </Group>
                              <Group
                                justify="space-between"
                              ...
                              >
                                ...
                                <Text
                                  ...
                                  c="dimmed"
                                >
                                  {selectedCustomerCredit.credit_days} days
                                </Text>
                              </Group>
                            </Paper>
                          )}
                    ...
                    {/* Payment method buttons (show for wholesale or retail with credit) */}
                    {saleType === 'wholesale' && (
                      <Group
                        gap="xs"
                        wrap="nowrap"
      ...
      <Modal
        opened={showCreditPaymentModal}
        onClose={() => setShowCreditPaymentModal(false)}
        title="Record Credit Payment"
        size="md"
      >
        {selectedCustomerCredit && selectedCustomerCredit.credit_limit > 0 && (
          <form onSubmit={handleCreditPayment}>
            <Stack gap="md">
              <Paper
                p="md"
                withBorder
              ...
              >
                ...
                <Text
                  ...
                  mt={4}
                >
                  Credit Limit: {defaultCurrency}{' '}
                  {selectedCustomerCredit.credit_limit.toLocaleString()}
                </Text>
              </Paper>
              ...
              <NumberInput
                ...
                required
                min={0.01}
                max={selectedCustomerCredit.current_credit_balance}
                thousandSeparator=","
                description={`Max: ${defaultCurrency} ${selectedCustomerCredit.current_credit_balance.toLocaleString()}`}
              />
              <Select
