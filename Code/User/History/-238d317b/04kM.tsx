...
const PayCustomerCreditModal = ({ customer }: PayCustomerCreditModalProps) => {
    ...
    validate: {
      amount: (value) => {
        ...
        if (value > outstandingBalance + 0.01)
          return `Amount cannot exceed outstanding balance of ${formatCompactCurrency(outstandingBalance, currency)}`
        return null
      },
  ...
  const handleSubmit = async (values: PaymentFormValues) => {
    try {
      ...
      // Update linked credit sale(s): mark as paid when customer fully clears balance
      // Find all pending credit sales for this customer ordered by oldest first
      const { data: creditSales } = await supabase
        .from('sales')
        .select('id, total_amount, credit_transaction_id')
      ...
      if (creditSales && creditSales.length > 0) {
        // Mark credit transactions as completed
        await supabase
          .from('credit_transactions')
          .update({
            status: 'completed',
      ...
      notifications.show({
        ...
        message: `Payment of ${formatCompactCurrency(amountPaid, currency)} recorded for ${customer.first_name} ${customer.last_name}. ${
          fullyPaid
            ? 'Balance fully cleared! ✅'
            : `Remaining: ${formatCompactCurrency(newBalance, currency)}`
        }`,
        color: 'green',
        autoClose: 6000,
      })
            ...
            <Stack gap="xs">
              <Group justify="space-between">
                ...
                <Text
                  ...
                  c="red.7"
                >
                  {formatCompactCurrency(outstandingBalance, currency)}
                </Text>
              </Group>
              <Group justify="space-between">
                <Text
              ...
              <Stack gap="xs">
                <Group justify="space-between">
                  ...
                  <Text
                    ...
                    c="green.7"
                  >
                    {formatCompactCurrency(newBalancePreview, currency)}
                  </Text>
                </Group>
                <Group justify="space-between">
                  <Text
                  ...
                  <Text
                    ...
                    c="green.7"
                  >
                    {formatCompactCurrency(newAvailablePreview, currency)}
                  </Text>
                </Group>
                <Group
                  justify="space-between"
        ...
        >
          ...
          <Stack gap="md">
            <NumberInput
              ...
              description={`Maximum: ${formatCompactCurrency(outstandingBalance, currency)}`}
            />
            <Select
