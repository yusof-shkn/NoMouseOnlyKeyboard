...
if the payment method is insurance it should check the insurance selected that its verified or not no need for other things 
**2. Pricing math bug for insured orders — delivery fee is always charged to insurance even the rider is selected walk in its wrong at first we should not show the delivery fee remove them at all orderr details and we will show it when we select rider and the rider accepted and the price if fixed 10k covered by insurance**
...
so the pharmacist can mark the order as delivered without any proof of handover. This is a major loophole for potential fraud, theft, or misdelivery. The system has no audit trail for these walk-in orders. we need to add a mandatory OTP verification step for walk-in pickups, generate an OTP at dispatch, and require the pharmacist to verify it at handover. This would create an audit trail and reduce the risk of fraud.
