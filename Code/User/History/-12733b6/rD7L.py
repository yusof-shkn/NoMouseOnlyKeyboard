...
# ─────────────────────────────────────────────
# 5. FETCH NDA GLOBAL PRODUCTS  (company_id = 1)
#    Never insert — just reference the existing NDA catalogue.
#    We pick a curated set of 15 common generics, one product per generic.
#    Prices are estimated since NDA products have null standard_cost/price.
# ─────────────────────────────────────────────

# Curated NDA product IDs (verified from DB) with estimated prices for batches/sales
# Format: (product_id, est_cost_price, est_selling_price)
NDA_PRODUCT_PRICE_MAP: dict[int, tuple[int, int]] = {
    ...
    1500: (120, 250),  # AB-DOX-100 — DOXYCYCLINE (already above, deduped)
}

# Deduplicated ordered list
NDA_PRODUCT_IDS: list[int] = list(dict.fromkeys(NDA_PRODUCT_PRICE_MAP.keys()))


def fetch_nda_products() -> list:
    """
    Fetch the curated NDA global products (company_id=1).
    Returns list of product dicts augmented with est_cost_price / est_selling_price
    for use in inventory and sales seeding.
    No INSERT — these products already exist globally.
    """
    print("\n💊  Fetching NDA global products (company_id=1)...")
    try:
        result = (
            supabase.table("products")
            .select(
                "id, product_name, generic_name, product_code, category_id, unit_id, reorder_level"
            )
            .eq("company_id", 1)
        ...
        print(f"  ⚠️   Failed to fetch NDA products: {e}")
        return []

    # Augment with estimated prices
    enriched = []
    for r in rows:
        cost, price = NDA_PRODUCT_PRICE_MAP.get(r["id"], (500, 1000))
        enriched.append({**r, "cost_price": cost, "selling_price": price})

    print(f"  ✅  {len(enriched)} NDA products loaded (read-only, no insert)")
    return enriched
...
def run_seed(args):
    ...
    # Step 6: Fetch NDA global products (company_id=1) — never insert
    if seed_all or getattr(args, "products", False):
        product_rows = fetch_nda_products()
    else:
        # Try company-specific first, fall back to NDA global
...
def main():
    ...
    parser.add_argument(
        "--products", action="store_true", help="Load NDA global products (no insert)"
    )
