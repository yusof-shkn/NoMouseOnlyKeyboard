...
/**
 * Column definitions for Cash Flow table.
 * All field accessors use actual v_cash_flow column names.
 */
export const getCashFlowColumns = (
  ...
  return [
    {
      header: 'Date',
      accessor: 'transaction_date', // ✅ was: entry_date
      width: '10%',
      minWidth: 100,
        ...
        <Box>
          ...
          >
            {row.transaction_date
              ? formatDate(row.transaction_date, currency)
              : '-'}
          </Text>
          {row.reference_type && (
            <Text
    ...
    },
    {
      ...
      render: (row: CashFlowTransaction) => (
        <Box style={{ minWidth: 0 }}>
          ...
          {/* reference_number is the receipt/ref field in v_cash_flow */}
    ...
    {
      header: 'Type',
      accessor: 'transaction_type', // ✅ real column in v_cash_flow
      width: '10%',
      minWidth: 90,
      render: (row: CashFlowTransaction) => (
        <Badge
          variant="light"
          color={row.transaction_type === 'inflow' ? 'green' : 'red'}
          size="sm"
          style={{ textTransform: 'capitalize', fontWeight: 600 }}
        >
          {row.transaction_type === 'inflow' ? 'Cash In' : 'Cash Out'}
        </Badge>
      ),
    },
    {
      header: 'Cash In',
      accessor: 'cash_in', // derived in enrichCashFlowTransactions
      width: '13%',
      minWidth: 120,
      render: (row: CashFlowTransaction) => {
        // Prefer enriched cash_in; fall back to deriving inline
        const cashIn =
          row.cash_in != null
            ? parseFloat(String(row.cash_in))
    ...
    {
      header: 'Cash Out',
      accessor: 'cash_out', // derived in enrichCashFlowTransactions
      width: '13%',
      minWidth: 120,
      render: (row: CashFlowTransaction) => {
        const cashOut =
          row.cash_out != null
            ? parseFloat(String(row.cash_out))
    ...
    {
      header: 'Amount',
      accessor: 'amount', // ✅ real column in v_cash_flow
      width: '13%',
      minWidth: 120,
      render: (row: CashFlowTransaction) => {
        const amount = parseFloat(String(row.amount ?? 0))
        const isInflow = row.transaction_type === 'inflow'
        return (
          <Text
            size="sm"
            fw={700}
            c={isInflow ? 'teal' : 'orange'}
            style={{ fontVariantNumeric: 'tabular-nums' }}
          >
            {isInflow ? '+' : '-'}
    ...
    },
    {
      ...
      accessor: 'running_balance', // derived in enrichCashFlowTransactions
      width: '14%',
      minWidth: 130,
      render: (row: CashFlowTransaction) => {
    ...
    {
      header: 'Payment',
      accessor: 'payment_method', // ✅ real column in v_cash_flow
      width: '10%',
      minWidth: 100,
      render: (row: CashFlowTransaction) => (
