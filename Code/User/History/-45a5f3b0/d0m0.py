...
# ── 4. Customers ──────────────────────────────────────────────────────────────
# FIX: first_name + last_name only (no customer_name, no gender, no date_of_birth)
...
def seed_purchases(
    ...
    for s_idx, store in enumerate(store_rows):
        ...
        po_rows = _rows(db_insert("purchase_orders", po_data))
...
# FIX: expenses uses category_id (not expense_category_id)
...
def seed_accounting(company_id: int, store_rows: list[Row]) -> None:
    ...
    for store in stores[:2]:
        ...
        if expenses:
            ...
            total += len(inserted)
            log_ok(f"  {len(inserted)} expenses for {store_name}.")
