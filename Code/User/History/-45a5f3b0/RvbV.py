...
is_restricted SEEDING:
  Every seeded record is split ~50/50 between is_restricted=True and is_restricted=False
  so the frontend toggle has meaningful data to show/hide.
  Affected tables: sales, sale_items, purchase_orders, purchase_order_items,
                   purchase_returns, sales_returns, expenses, income,
                   product_batches, stock_adjustments, products (NDA read-only, skipped)
...
def _restricted(idx: int) -> bool:
    """Return True for every other record so we get a 50/50 split."""
    return idx % 2 == 1
...
def seed_stores(company_id: int) -> tuple[list[Row], list[Row]]:
    ...
    area_rows = _rows(db_insert("areas", areas_data))
    if not area_rows:
    ...
    store_rows = _rows(db_insert("stores", stores_data))
...
def seed_suppliers(company_id: int) -> list[Row]:
    ...
    rows = _rows(db_insert("suppliers", data))
...
def seed_customers(company_id: int) -> list[Row]:
    ...
    rows = _rows(db_insert("customers", data))
...
def fetch_nda_products() -> list[Row]:
    ...
    restricted_count = sum(1 for r in enriched if r.get("is_restricted"))
    log_ok(
        f"{len(enriched)} NDA products loaded (no insert). "
        f"{restricted_count} are is_restricted=True."
    )
    return enriched
...
def seed_inventory(
    ...
    batches: list[Row] = []
    batch_idx = 0  # global counter for is_restricted alternation
            ...
            batches.append(
                {
                    ...
                    "supplier_id": sup,
                    "is_active": True,
                    "is_restricted": _restricted(batch_idx),
                }
            )
            batch_idx += 1

    # Near-expiry demo batch (always not restricted so it's always visible)
    p0 = product_rows[0]
    p0_cost = int(p0.get("unit_cost") or 500)
    p0_sell = int(p0.get("selling_price") or 1000)
    batches.append(
        {
            "company_id": c,
    ...
    rows = _rows(db_insert("product_batches", batches))
    restricted_count = sum(1 for b in batches if b.get("is_restricted"))
    log_ok(
        f"{len(rows)} batches inserted "
        f"({len(product_rows)} products x {len(store_rows)} stores + 1 near-expiry). "
        f"{restricted_count} restricted, {len(rows) - restricted_count} unrestricted."
    )
    return rows
...
def seed_purchases(
    ...
    total_po = 0
    po_global_idx = 0  # for is_restricted alternation
    ...
    for s_idx, store in enumerate(store_rows):
        ...
        # 3 purchase orders per store, alternating is_restricted
        po_defs = [
            {
                "company_id": c,
                "store_id": sid,
                ...
                "is_restricted": _restricted(po_global_idx),
            },
            {
                "company_id": c,
                "store_id": sid,
                ...
                "payment_method": "cash",
                "is_restricted": _restricted(po_global_idx + 1),
            },
            {
                "company_id": c,
                "store_id": sid,
                ...
                "payment_method": "bank_transfer",
                "is_restricted": _restricted(po_global_idx + 2),
            },
        ]
        po_rows = _rows(db_insert("purchase_orders", po_defs))
        ...
        if po_rows and len(product_rows) >= 2:
            ...
            # purchase_order_items inherit is_restricted from their parent PO
            items: list[Row] = [
                {
    ...
    restricted_pos = sum(1 for d in range(len(store_rows) * 3) if _restricted(d))
    log_ok(
        f"{total_po} purchase orders inserted (3 per store). "
        f"~{restricted_pos} restricted, ~{total_po - restricted_pos} unrestricted."
    )
...
def seed_sales(
    ...
    inst = next(
        (x for x in customer_rows if "INST" in str(x.get("customer_code", ""))),
        cust1,
    )

    batch_map: dict[int, int] = {int(b["product_id"]): int(b["id"]) for b in batch_rows}
    total_sales = 0
    sale_global_idx = 0  # for is_restricted alternation
    ...
    for s_idx, store in enumerate(store_rows):
                ...
                sales_list.append(
                    {
                        ...
                        "payment_status": "paid",
                        "is_restricted": _restricted(sale_global_idx),
                    }
                )
                sale_global_idx += 1

        # 1 credit / institutional sale (always restricted=False so it's always visible)
        sales_list.append(
            {
                "company_id": c,
                "store_id": sid,
        ...
        sale_rows = _rows(db_insert("sales", sales_list))
        ...
        if sale_rows and len(product_rows) >= 2:
            ...
            for sr_idx, sr in enumerate(sale_rows[:3]):
                ...
                sale_id = int(sr["id"])
                # sale_items inherit is_restricted from the parent sale
                parent_restricted = sales_list[sr_idx].get("is_restricted", False)
                items += [
                    {
                        "sale_id": sale_id,
                        "product_id": p1_id,
    ...
    restricted_sales = sum(1 for s in range(sale_global_idx) if _restricted(s))
    log_ok(
        f"{total_sales} sales inserted (~{14 * 3 + 1} per store x {len(store_rows)} stores). "
        f"~{restricted_sales} restricted, ~{total_sales - restricted_sales} unrestricted."
    )
...
def seed_accounting(company_id: int, store_rows: list[Row]) -> None:
    ...
    exp_cats = _rows(exp_cats_raw)

    if not exp_cats:
        log_warn(
            "No expense categories found -- skipping accounting. Run --copy-template first."
        )
        return
    ...
    cat_map: dict[str, int] = {str(r["category_name"]): int(r["id"]) for r in exp_cats}
    ...
    templates: list[tuple[str, int, str, bool, str | None]] = [
    ...
    total = 0
    exp_global_idx = 0  # for is_restricted alternation
    ...
    for store in stores[:2]:
        ...
        for idx, (label, amount, method, recurring, freq) in enumerate(templates):
            ...
            row: Row = {
                ...
                "payment_status": "paid",
                "is_restricted": _restricted(exp_global_idx),
            }
            exp_global_idx += 1
        ...
        if expenses:
            inserted = _rows(db_insert("expenses", expenses))
            total += len(inserted)
            restricted_count = sum(1 for e in expenses if e.get("is_restricted"))
            log_ok(
                f"  {len(inserted)} expenses for {store_name} "
                f"({restricted_count} restricted)."
            )
...
def clear_seed_data(company_id: int) -> None:
    ...
    try:
        ...
        print(f"  deleted: companies (id={c})")
    except Exception as e:
        print(f"  skipped companies: {e}")
