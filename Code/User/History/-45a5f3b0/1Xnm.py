...
def seed_inventory(
    ...
    po_ids_by_store: dict[int, int] | None = None,
) -> list[Row]:
    """
    Seed inventory batches.

    po_ids_by_store: optional dict of {store_id: purchase_order_id} so that
    each store's initial batch is linked to its first (received) PO.
    This is populated by seed_purchases() and passed in from runner.py.
    If not provided batches are inserted without a purchase_order_id.
    """
    c = company_id
    ...
    for s_idx, store in enumerate(store_rows):
        ...
        # Link to the first received PO for this store (if available)
        po_id = (po_ids_by_store or {}).get(sid)
    ...
    first_store_id = int(store_rows[0]["id"])
    if (po_ids_by_store or {}).get(first_store_id):
        near_expiry["purchase_order_id"] = po_ids_by_store[first_store_id]  # type: ignore[index]
    batches.append(near_expiry)
...
def seed_purchases(
    ...
    """
    Seed purchase orders (3 per store: received / partial / pending).

    Returns:
        po_ids_by_store: {store_id: first_received_po_id}
        Pass this to seed_inventory() so product_batches.purchase_order_id is populated.
    """
    c = company_id
    ...
    total_po = 0
    po_idx = 0
    # Maps store_id → id of the first received PO (for batch linkage)
    po_ids_by_store: dict[int, int] = {}
    ...
    for s_idx, store in enumerate(store_rows):
        ...
        # Record the first received PO so seed_inventory can link batches to it
        if po_rows:
            po_ids_by_store[sid] = int(po_rows[0]["id"])
...
def clear_seed_data(company_id: int) -> None:
    ...
    print("\n  Deleting child tables via parent IDs...")
    for child_tbl, parent_tbl, child_fk, parent_col in indirect_deletes:
        try:
            ...
            print(f"    OK  {child_tbl}")
        except Exception as exc:
            print(f"    WARN  {child_tbl}: {exc}")
    ...
    try:
        ...
        print(f"    OK  companies (id={c})")
    except Exception as exc:
        print(f"    WARN  companies: {exc}")
