...
FIX LOG:
  ...
  - sale_items, sales_return_items, purchase_return_items, purchase_order_items,
    customer_insurance, supplier_ratings: no company_id column,
    cleared via JOIN-delete through parent tables (handled in seed_runner.py)
    role_permissions: NOT cleared — roles are global at company_id=1
"""
...
def seed_accounting(company_id: int, store_rows: list[Row]) -> None:
    ...
    # ── Expenses ──────────────────────────────────────────────────────────────
    # expense_categories are global — query from company_id=1
    exp_cats = _rows(
        get_client()
    ...
    if not exp_cats:
        log_warn(
            ...
            "Run: python main.py template seed"
        )
    else:
    ...
    # ── Income ────────────────────────────────────────────────────────────────
    # income_categories are global — query from company_id=1
    inc_cats = _rows(
        get_client()
...
def clear_seed_data(company_id: int) -> None:
    ...
    # GLOBAL tables are intentionally excluded — they live at company_id=1
    # and are shared by all companies. Never delete them per company.
    # Excluded: units, roles, categories, chart_of_accounts,
    #           income_categories, expense_categories
    direct_tables = [
        "sale_payments",
        "sales_returns",  # children (sales_return_items) deleted below
        "sales",  # sale_items deleted via JOIN below
        "purchase_returns",
    ...
    # role_permissions excluded — roles are global, don't touch them.
    indirect_deletes = [
