...
Now I have a thorough understanding of the codebase. Let me analyze all tasks and create a structured plan.Now I have a solid understanding of the project. Let me compile and present the full task breakdown.I now have enough context. Let me present the full task analysis.Now I have a comprehensive understanding of the codebase. Let me present the organized task breakdown clearly.

---

Here's my analysis of the CLAUDE.md and all tasks. I've investigated the codebase thoroughly to understand what exists and what needs to be built.

## Task Breakdown — Sorted by Priority & Dependency

---

### 🔴 PART 1 — Bug Fixes (High Priority, Breaks Core Functionality)

These are existing bugs that break or hide data from users:

1. **Email & Date of Birth not appearing in Edit Profile** — The fields *are* in the code (lines 304–345 of EditProfile.tsx), but something is hiding them. Likely a rendering/condition issue to investigate and fix.

2. **Scheme expiry logic not implemented** — `InsuranceSchemes.tsx` saves `expiry_date` to DB, but there's no enforcement: the scheme stays active past its expiry on the customer side and in UploadPrescription. Need to check and auto-deactivate expired schemes.

3. **Enable/disable active status bug in Scheme Management** — The `is_active` Switch in InsuranceSchemes is broken. Need to debug and fix the toggle logic.

4. **Address modal — rename "Label" to "Location Name"** — Simple label rename in `AddressPickerModal.tsx`.

5. **City input should be a dropdown** — Change the city `TextInput` in AddressPickerModal to a `Select` with Uganda cities/towns.

6. **In Edit Profile, adding insurance should not require "Save Changes"** — Insurance save should be independent (fire immediately), not gated on the main form save button.

---

### 🟡 PART 2 — Missing Fields & Display Fixes

7. **Approval number for approvals** — `InsuranceApprovals.tsx` doesn't show an approval/reference number. Need to add this field.

8. **Order numbers** — Verify `order_number` is displayed consistently across `InsuranceOrders.tsx` and `InsuranceOrderDetail.tsx`.

9. **Show chronic cases on Insurance Verification page** — `InsuranceVerifications.tsx` needs a "chronic" indicator/badge when `profiles.is_chronic = true`.

10. **Delivery fee covered by insurance** — In `PharmacyOrderDetail.tsx`, the `insuredAmount` calculation already includes delivery fee for non-cash orders (line 548), but the insurance portal's order view may not reflect this. Need to verify `InsuranceOrderDetail.tsx`.
...
16. **Actor/Auditor section** — New section to audit who did what (staff actions log).

17. **Report section** — Reporting/analytics page for insurance or pharmacy.

18. **Delivery schedule** — A schedule view for deliveries.

19. **Place order on behalf of customer** — Pharmacy staff can initiate an order for a customer from the pharmacy portal.
...
**Shall I start Part 1 now?** I'll fix all 6 bugs: email/DOB visibility in Edit Profile, scheme expiry enforcement, is_active toggle bug, address modal label rename, city dropdown, and auto-save insurance in Edit Profile — then package a `done.zip`.
