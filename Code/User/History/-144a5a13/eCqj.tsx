...
export function CustomerLayout({ children }: CustomerLayoutProps): JSX.Element {
  ...
  // Fires once per page load — prevents re-showing when selectedProfile updates mid-session
  const insuranceModalFiredRef = useRef(false)
  // Track IDs of notifications already seen so we can toast only new ones
  ...
  // Show no-insurance modal on every login until the user adds insurance.
  // Fires once per page load (insuranceModalFiredRef prevents re-triggering
  // when selectedProfile refreshes mid-session after insurance data loads).
  useEffect(() => {
    if (!user || !selectedProfile) return
    if (insuranceModalFiredRef.current) return
    const insStatus =
      (selectedProfile.insurance as { status?: string } | null)?.status ?? 'none'
    // Only show when there is genuinely no insurance on this profile
    if (insStatus && insStatus !== 'none') return
    // Mark as fired so subsequent profile refreshes don't re-trigger
    insuranceModalFiredRef.current = true
    // Small delay so the page finishes rendering before the modal appears
    const t = setTimeout(() => setNoInsuranceOpen(true), 1500)
  ...
  const fetchNotifs = (): void => {
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        if (isFirstLoad) {
          // Seed all current IDs — don't toast on initial page load
          incoming.forEach(n => seenNotifIdsRef.current.add(n.id))
          return
        }

        // Toast only unread notifications that we haven't seen before
        for (const n of incoming) {
          if (!n.is_read && !seenNotifIdsRef.current.has(n.id)) {
            seenNotifIdsRef.current.add(n.id)
            const color =
              n.type === 'insurance_verified' ? 'teal' :
              n.type === 'insurance_rejected' ? 'red' :
              n.type === 'delivered'           ? 'teal' :
              n.type === 'pricing_ready'       ? 'orange' :
              n.type === 'out_for_delivery'    ? 'cyan' :
              n.type === 'delivery_request'    ? 'violet' :
              'blue'
  ...
  useEffect(() => {
    ...
    // Fallback polling every 2 seconds — catches missed realtime events
    const poll = setInterval(fetchNotifs, 2000)
      ...
      >
        <Box>
          ...
          <Stack gap="xs" mt="lg">
            ...
            <Button variant="subtle" color="gray" fullWidth onClick={() => setNoInsuranceOpen(false)}>k              Remind me later
            </Button>
          </Stack>
        </Box>
      </Modal>
