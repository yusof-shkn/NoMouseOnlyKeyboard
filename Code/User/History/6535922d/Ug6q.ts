...
// ✅ CLEANED: Removed partiallyReceived and partiallyPaid from stats interface
...
export const handleReceiveGoods = async (
  ...
  // Open the ReceiveGoodsForm modal instead of using confirm()
  dispatch(
    openModal({
      type: 'receive-goods', // register this key in your generic modal switch
      size: 'lg',
      props: {
...
export const handleCheckReturnEligibility = async (
  ...
  try {
    console.log('🔍 Checking return eligibility for PO:', {
    ...
    if (!row.company_id) {
      console.error('❌ Missing company_id in row object:', row)
      notifications.show({
    ...
    // ✅ CLEANED: Only 'received' status can be returned
    ...
    console.log('✅ Opening return modal with data:', {
      companyId: row.company_id,
      storeId: row.store_id,
      supplierId: row.supplier_id,
      poId: row.id,
...
export const handleCreateReturn = async (row: any, navigate: any) => {
  try {
    ...
    console.log('📝 Creating return for PO:', row.po_number)
...
export const handleEdit = async (
  ...
  try {
    console.log('🔍 Edit clicked for PO:', {
      po_number: row.po_number,
      status: row.status,
      store_id: row.store_id,
      store_name: row.store_name,
    ...
    if (!poStoreId || poStoreId === null || poStoreId === undefined) {
      console.error('❌ Invalid store_id in PO:', row)
      notifications.show({
        title: 'Invalid Purchase Order',
    ...
    console.log('🏪 Validating store with ID:', poStoreId)
    ...
    if (storeError) {
      console.error('❌ Error validating store:', storeError)
      notifications.show({
    ...
    if (!storeCheck) {
      console.error('❌ Store not found or deleted for ID:', poStoreId)
      notifications.show({
    ...
    if (!isAdmin && currentStoreId !== poStoreId) {
      notifications.show({
        title: 'Access Denied',
        message: `You don't have access to edit purchase orders from ${poStoreName}. Please contact your administrator.`,
        color: 'red',
        autoClose: 6000,
      })
      return
...
// ✅ CLEANED: Only 'received' status can be marked as paid
...
export const submitSupplierRating = async (ratingData: {
  ...
  try {
    ...
    console.log('📊 Submitting rating data:', ratingInsertData)
    ...
    console.log('✅ Rating record created:', ratingRecord)
    ...
    if (fetchError) {
      console.error('❌ Error fetching supplier ratings:', fetchError)
      throw fetchError
    }
    ...
    console.log('✅ Supplier rating submitted successfully:', {
      supplier_id: ratingData.supplierId,
