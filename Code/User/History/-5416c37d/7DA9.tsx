...
const statusColor = (s: string): string =>
  ({
    ...
    packing: '#8B5CF6',
    dispatched: '#6366F1',
    pharmacy_review: '#0EA5E9',
    prescription_uploaded: '#64748B',
...
const Pill = styled.button<{ $on: boolean }>`
  ...
  border-radius: 28px;
  border: 1.5px solid
    ${(p) => (p.$on ? 'rgba(0,103,130,0.70)' : 'rgba(196,198,210,0.30)')};
...
const Card = styled.div<{ $hot: boolean; $i: number }>`
  ...
  box-shadow: 0 4px 24px rgba(1, 41, 112, 0.1);
...
export default function OrderHistory(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const fetch = (): void => {
      let q = supabase
        .from('orders')
        .select(
          '*, medications(*), profiles(name), selected_insurance:insurance!orders_selected_insurance_id_fkey(provider, scheme_name, status)',
        )
        .eq('user_id', user.id)
    ...
    fetch()
    const ch = supabase
      .channel(`oh:${user.id}:${viewAll}:${pid ?? 'a'}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const filtered = orders.filter(
    (o) =>
      (!filter ||
        (filter === '_active_' ? isLive(o.status) : o.status === filter)) &&
      (!search ||
  ...
  const currentProfileLabel = viewAll
    ? 'All profiles'
    : (profiles.find((p) => p.id === filterProfileId)?.name ??
      activeProfile?.name ??
      'Profile')
                    ...
                    <Orb>
                      {order.status === 'delivered' ? (
                        ...
                        <TbX
                          ...
                          color="#9CA3AF"
                        />
                      ) : ['out_for_delivery', 'dispatched'].includes(
                          order.status,
                        ) ? (
                        <TbTruck
                          size={19}
        ...
        <Stack gap="xs">
          ...
          >
            <Group gap={8}>
              <TbUsers
                size={15}
                color={viewAll ? SavedColors.Primaryblue : '#9CA3AF'}
              />
              <Text
                size="sm"
                fw={600}
                c={viewAll ? SavedColors.Primaryblue : SavedColors.TextColor}
              >
                All profiles
              </Text>
            </Group>
      ...
      <Modal
        ...
        title={
          <Group gap="xs">
            ...
            <Text
              fw={700}
              style={{ fontFamily: "'Manrope', sans-serif", color: '#171c20' }}
            >
        ...
        {reorderSrc && (
          <ScrollArea.Autosize mah={520}>
            <Stack gap="md">
              ...
              {((reorderSrc.medications as DbMedication[] | null)?.length ??
                0) > 0 && (
                <Box>
                  <Divider
                    mb="sm"
              ...
              {addrSnap?.street && (
                <Box>
                  ...
                  >
                    ...
                    {addrSnap.street}
                    {addrSnap.city ? `, ${addrSnap.city}` : ''}
                    {addrSnap.zone ? ` (${addrSnap.zone})` : ''}
                  </Text>
                </Box>
              )}
              ...
              >
                <Text
                  ...
                  c="#92400e"
                >
                  A new prescription image is required. Upload a fresh
                  prescription — all other details will be pre-filled from this
                  order.
                </Text>
              </Box>
              <Group
              ...
              >
                <Button
                  variant="default"
                  radius={8}
                  onClick={() => setReorderSrc(null)}
                >
                  Cancel
                </Button>
                <Button
                  radius={8}
                  leftSection={<TbRepeat size={15} />}
