...
/**
 * Get balance sheet data using the vw_balance_sheet database view
...
export const getBalanceSheet = async ({
  ...
  try {
    ...
    // Query the vw_balance_sheet view — fetch ALL rows first (no server pagination)
    ...
    // Ordering by type_order (built into view) and account code
    query = query
      .order('type_order', { ascending: true })
      .order('subtype_order', { ascending: true })
...
/**
 * Get balance sheet statistics using the vw_balance_sheet view
 */
export const getBalanceSheetStats = async (
  ...
  try {
    ...
    // Query the vw_balance_sheet view for statistics
    const { data, error } = await supabase
      .from('vw_balance_sheet')
      .select('*')
      .eq('company_id', companyId)
    ...
    // Parse and calculate totals — always use parseFloat since Supabase returns numerics as strings
    const totalAssets = entries
      .filter((e: any) => e.account_type === 'asset')
      .reduce(
        (sum: number, e: any) => sum + Math.abs(parseFloat(e.balance || '0')),
        0,
      )
