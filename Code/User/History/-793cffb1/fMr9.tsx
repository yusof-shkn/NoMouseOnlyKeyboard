...
import { PurchaseOrdersDrawer } from './components/PurchaseOrdersDrawer'
...
export const PurchasePOP: React.FC = () => {
        ...
        try {
          ...
          if (items && items.length > 0) {
            const loadedItems = items.map((item: any) => ({
              ...
              unit:
                item.product.unit?.short_code || item.product.unit?.name || '',
  ...
  const onAddProduct = (product: any) => {
    if (receiveImmediately) {
      ...
      const _unitObj =
        product.unit && typeof product.unit === 'object' ? product.unit : null
      const tempItem = {
        id: product.id,
  ...
  const onSaveDraft = async () => {
    ...
    if (!selectedSupplier) {
      notifications.show({
        ...
        message: 'Please select a supplier before saving as draft',
        color: 'red',
      })
      return
    }
  ...
  return (
    ...
    >
              ...
              >
                ...
                {categories.map((category: Category) => (
                  <Button
                    ...
                    size="xs"
                    variant={
                      selectedCategoryId === category.id ? 'filled' : 'light'
                    }
                    color={theme.primaryColor}
      ...
      {/* ── Fixed Purchase Orders button ─ bottom left ────────────────── */}
      <Box
        style={{
          position: 'fixed',
          bottom: 24,
      ...
      >
        <Tooltip
          label="View my purchase orders for this store"
          position="right"
          withArrow
        >
          <Button
      ...
      <PurchaseOrdersDrawer
        opened={ordersDrawerOpen}
        onClose={() => setOrdersDrawerOpen(false)}
        storeId={safeStoreId}
        userId={authUser?.profile?.id ?? null}
        companyId={safeCompanyId}
        currency={currency}
        onResumeDraft={(po) => {
          // Reset then load draft PO items — same mapping as edit mode loader
          onResetOrder()
          const items = (po.purchase_order_items || []).map((item: any) => ({
            id: item.product_id,
            productName: item.product?.product_name || '',
            productCode: item.product?.product_code || '',
            genericName: item.product?.generic_name || '',
            unit:
              item.product?.unit?.short_code || item.product?.unit?.name || '',
          ...
          setOrderItems(items)
          if (po.supplier_id) setSelectedSupplier(String(po.supplier_id))
          if (po.po_date) setOrderDate(po.po_date.slice(0, 10))
          if (po.discount_amount) setDiscount(po.discount_amount)
          if (po.payment_method) setSelectedPayment(po.payment_method as any)
          setEditMode(true)
          setEditingPOId(po.id)
          setEditingPOStatus('draft')
          notifications.show({
            title: 'Draft Resumed',
            message: `${po.po_number} loaded — complete when ready`,
            color: 'yellow',
          })
        }}
      />
