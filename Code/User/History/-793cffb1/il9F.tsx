...
export const PurchasePOP: React.FC = () => {
  ...
  // ── FIXED: onSubmitOrder now builds invoiceMeta and passes it through ──────
  const onSubmitOrder = async (receiveInvoiceNumber?: string) => {
    setIsSubmitting(true)

    // The modal passes the invoice number entered inside it when
    // receiveImmediately is on; fall back to the POP-level invoice field.
    const effectiveInvoiceNumber = receiveInvoiceNumber?.trim() || invoiceNumber

    // ── Build InvoiceMeta from the effective invoice number ────────────────
    const invoiceMeta =
      receiveImmediately && effectiveInvoiceNumber
        ? {
