...
`
const BarSpacer = styled.div<{ $h: number }>`
...
const ChatFloat = styled.button<{ $barVisible: boolean }>`
  ...
  right: 18px;
  z-index: 400;
  bottom: ${(p: { $barVisible: boolean }) =>
    p.$barVisible ? 'calc(64px + 130px)' : '84px'};
  width: 54px;
  height: 54px;
  border-radius: 50%;
  border: none;
...
export default function InsuranceOrderDetail(): JSX.Element {
  ...
  const handleApprove = async (): Promise<void> => {
    ...
    setOrder((prev) =>
      prev ? { ...prev, status: 'awaiting_confirmation' } : null,
    )
    notifications.show({
      title: '✅ Approved',
        ...
        <Stack gap="lg">
          ...
          {meds.length > 0 && (
            ...
            >
                      ...
                      <Table.Tr key={m.id}>
                        ...
                        <Table.Td ta="right">
                          <Text
                            ...
                            fw={600}
                          >
                            UGX {(m.unit_price * m.quantity).toLocaleString()}
                          </Text>
                        </Table.Td>
                        <Table.Td ta="center">
                          <Badge
              ...
              <Box
                p="md"
                style={{ borderTop: `1px solid ${SavedColors.DemWhite}` }}
              >
                <Stack gap={4}>
                  ...
                  <Group justify="space-between">
                    ...
                    <Text
                      size="sm"
                      c={
                        (order as { delivery_skipped?: boolean })
                          .delivery_skipped
                          ? 'dimmed'
                          : undefined
                      }
                  ...
                  {(order.insured_amount ?? 0) > 0 && (
                    <Group justify="space-between">
                      ...
                      <Text
                        ...
                        c="teal"
                      >
                        − UGX {(order.insured_amount ?? 0).toLocaleString()}
                      </Text>
                    </Group>
                  )}
                  <Divider my={4} />
          ...
          >
            <Box style={{ flex: 1, minWidth: 0 }}>
              ...
              <Text
                ...
                mt={1}
              >
                Review the bill above, then approve or reject.
              </Text>
            </Box>
            <Group
              gap={8}
